...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable
	{
		...
		private Body m_Body;
		private string m_BonusName; // Neotr - dodanie bonusow Naza
		private int m_BonusValue; // Neotr - dodanie bonusow Ilosc
		private int m_Hue;
		...
		private string m_Klasa; // Neotr - dodanie kals
		...
		{
			Race.MakeRandomAppearance(this);
		}

		#region Nelderim Labels

		[CommandProperty(AccessLevel.Administrator, true)]
		public string ModifiedBy {
			get { return Labels.Get(this).ModifiedBy; }
			set { Labels.Get( this ).ModifiedBy = value; }
		}

		[CommandProperty(AccessLevel.Administrator, true)]
		public DateTime ModifiedDate {
			get { return Labels.Get( this ).ModifiedDate; }
			set { Labels.Get( this ).ModifiedDate = value; }
		}

		private string[] m_Labels
		{
			get { return Labels.Get( this ).Labels; }
		...
		{
			get{ return Labels.Get( this ).Labels[1]; }
			set
			{
				Labels.Get( this ).Labels[1] = value; InvalidateProperties();
			}
		}
		...
		{
			get{ return Labels.Get( this ).Labels[2]; }
			set
			{
				Labels.Get( this ).Labels[2] = value; InvalidateProperties();
			}
		}
		...
		{
			get{ return Labels.Get( this ).Labels[3]; }
			set
			{
				Labels.Get( this ).Labels[3] = value; InvalidateProperties();
			}
		}
		...
		{
			get{ return m_Labels[4]; }
			set
			{
				m_Labels[4] = value; InvalidateProperties();
			}
		}
		...
		public virtual void AddLabelsProperty( ObjectPropertyList list )
		{
			...
			for ( int i = 0; i < m_Labels.Length - 2; i++ )
			{
				if ( m_Labels[i] != null && m_Labels[i] != "" )
				{
					if ( labels != null && labels != "" )
					{
						labels += String.Format( "\n{0}", m_Labels[i] );
					}
					else
					{
						labels += String.Format( "{0}", m_Labels[i] );
					}
				}
			}
        ...
        public void RemoveMaskOfDeathEffect()
        {
            _maskOfDeathEffect = false;
        }
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			switch( version )
			{
				...
				case 34:
				{
					if ( version < 37 )
					{
						/*m_HiddenGM = */reader.ReadBool();
						/*m_HiddenGMLevel = (AccessLevel)*/reader.ReadInt();
					}
						goto case 33;
				}
				case 33: 
				...
				case 29:
					{
						if ( version == 37 )
						{
							byte raceIndex = reader.ReadByte();
							m_Race = Race.Races[raceIndex + (raceIndex == 0 ? 0 : 0x20)];
						}
						else 
				...
				case 0:
					{
						...
						//{
						//	m_HiddenGM = false;
						//	m_HiddenGMLevel = AccessLevel.Player;
						//}
						...
						{
							m_BonusValue = reader.ReadInt(); // Neotr - dodanie bonusow Ilosc
							m_BonusName = reader.ReadString(); // Neotr - dodanie bonusow Nazwa
						}
						...
						if ( version > 31 && version < 39 )
							m_Klasa = reader.ReadString(); // Neotr - dodanie klas
			...
			Utility.Intern( ref m_BonusName ); // Neotr - dodanie bonusow Naza
			Utility.Intern( ref m_Title );
			Utility.Intern( ref m_Language );
			Utility.Intern( ref m_Klasa ); // Neotr - dodanie klas
		...
		public virtual void Serialize( GenericWriter writer )
		{
			...
			//writer.Write( (bool) m_HiddenGM );
			//writer.Write( (int) m_HiddenGMLevel );
			
			//writer.Write( m_Labels.Length ); // write length
			...
			//writer.Write( m_BonusName ); // Neotr - dodanie bonusow Nazwa
			//writer.Write( (int)m_BonusValue ); // Neotr - dodanie bonusow Ilosc
			writer.Write( m_Name );
			...
			//writer.Write( m_Klasa ); // Neotr - dodanie klas
		...
		public AccessLevel AccessLevel
		{
			get
			{
				return HiddenGM ? AccessLevel.Player : m_AccessLevel;
			}
			set
			{
		...
		{
			get { return m_HiddenGM; }
			set { m_HiddenGM = value; }
		}

		[CommandProperty( AccessLevel.Counselor, AccessLevel.Administrator )]
		public AccessLevel TrueAccessLevel
		{
			get
			{
				...
				if( m_Map != value )
				{
					...
					if( m_Map != null )
					{
						if( ns != null && m_Map != null )
						{
							...
							ns.Send( SeasonChange.Instantiate( GetSeason(), false ) );
		...
		public virtual bool CanSee( Mobile m )
		{
			...
			return this == m || (
				m.m_Map == m_Map &&
				(!m.Hidden || (m_AccessLevel != AccessLevel.Player && (TrueAccessLevel >= m.AccessLevel || m_AccessLevel >= AccessLevel.Developer))) &&
		...
		public string BonusName
		{
			...
			set
			{
				if( m_BonusName != value )
					m_BonusName = value;
			}
		}
		
		// Neotr - dodanie bonusow Ilosc
		[CommandProperty( AccessLevel.GameMaster )]
		...
		public Mobile()
		{
			...
			//m_Labels = new string[5]; // at OSI maximum known item's labels is 2, i reserved first element of array for prevent cheating from staff
        ...
        // 23.09.2012 :: zombie
        protected class InitTimer : Timer
        {
            private Mobile m_Target;

            public InitTimer( Mobile target ) : base( TimeSpan.FromMilliseconds( 100 ) )
            {
                m_Target = target;
            ...
            protected override void OnTick()
            {
                ...
                {
                    Console.WriteLine( "BaseCreature InitTimer error: {0}", e.ToString() );
                }
            }
        }
		...
		public virtual bool ShowFameTitle { get { return false; } }//(m_Player || m_Body.IsHuman) && m_Fame >= 10000; } 
		...
		public bool CanHearGhosts
		{
			get
			{
				return Skills[SkillName.SpiritSpeak].Base >= 100 || AccessLevel >= AccessLevel.Counselor;
