...
namespace Server
{
	...
	public partial class Item : IEntity, IHued, IComparable<Item>, ISerializable
	{
		...
		private string[] m_Labels
		{
			...
			set
			{
				Labels.Get( this ).Labels = value; InvalidateProperties();
			}
		}
		...
		{
			get{ return Labels.Get( this ).Labels[4]; }
			set
			{
				Labels.Get( this ).Labels[4] = value; InvalidateProperties();
			}
		}
		...
		{
			if ( dropped.Stackable && Stackable && dropped.GetType() == GetType() && dropped.ItemID == ItemID && dropped.Hue == Hue && dropped.Name == Name && (dropped.Amount + Amount) <= 60000 && dropped.LootType == this.LootType)
			{
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 6:
				{
					...
					if ( GetSaveFlag( flags, SaveFlag.Parent ) )
					{
						Serial parent = (Serial)reader.ReadInt();
