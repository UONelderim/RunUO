	...
	public class Container : Item
	{
		...
		public int ConsumeTotalGrouped( Type[][] types, int[] amounts, bool recurse, OnItemConsumed callback, CheckItemGroup grouper )
		{
			...
			for ( int i = 0; i < types.Length; ++i )
			{
                ...
                // Wyszukujemy w pojemniku wszystkie przedmioty spelniajace typ danego surowca
                // i grupujemy je wg hue.
                // groups: { {UncutCloth_red, Cloth_red}, {Cloth1_green}, {Cloth_blue, UncutCloth_blue} }
				List<List<Item>> groups = new List<List<Item>>();
				int idx = 0;
				...
				for ( int j = 0; j < groups.Count; ++j )
				{
                    ...
                    // i - ogolny surowiec (Log and Board)
                    // j - kolor
                    // k - typ przedmiotu (either Log or Board)
                    // totals[i][j] - sumaryczna ilosc surowca o danym kolorze
			...
			for ( int i = 0; i < items.Length; ++i )    // ogolne surowce
			{
				for ( int j = 0; j < items[i].Length; ++j ) // kolory
				{
					if ( totals[i][j] >= amounts[i] )   // ilosc surowca danego koloru jest wystarczajaca
					{
						int need = amounts[i];

						for ( int k = 0; k < items[i][j].Length; ++k )  // poszczegolne typy surowca danego koloru
						{
							Item item = items[i][j][k];
