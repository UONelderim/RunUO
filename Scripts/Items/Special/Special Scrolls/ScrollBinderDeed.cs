        ...
        public override void GetProperties(ObjectPropertyList list) {
            ...
            switch (BinderType) {
                ...
                case BinderType.PowerScroll: {
                        ...
                        list.Add(3010146, string.Format("{0}\t{1}\t{2}\t{3}", v.ToString(), skillName, ((int)Has).ToString(), Needed.ToString())); // ~1_bonus~ ~2_type~: ~3_given~/~4_needed~
                        break;
                    }
                case BinderType.StatScroll: {
                        list.Add(3010146, string.Format("+{0}\t#{1}\t{2}\t{3}", v - 225, 1049477, ((int)Has).ToString(), Needed.ToString())); // ~1_bonus~ ~2_type~: ~3_given~/~4_needed~
                        break;
                    }
                //case BinderType.SOT: {
            ...
            if (IsChildOf(from.Backpack)) {
                ...
                switch (BinderType) {
                    ...
                    case BinderType.None:
                        cliloc = 3010148; break; // Target the scroll you wish to bind.
                    case BinderType.PowerScroll:
                        cliloc = 3010149; break; // Target the powerscroll you wish to bind.
                    case BinderType.StatScroll:
            ...
            switch (BinderType) {
                case BinderType.None: {
                        if (targeted is PowerScroll) {
                            ...
                            if (ps.Value >= 120) {
                                from.SendLocalizedMessage(1060634); // This scroll is already the highest of its type and cannot be bound.
                                return;
                            }
                        ...
                        //    sot.Delete();
                            ...
                            from.SendLocalizedMessage(1060633); // You may only bind powerscrolls, stats scrolls or scrolls of transcendence.
                        }
                ...
                case BinderType.PowerScroll: {
                        if (targeted is PowerScroll) {
                            ...
                            if (ps.Skill != Skill || ps.Value != Value) {
                                from.SendLocalizedMessage(1060632); // This scroll does not match the type currently being bound.
                                return;
                            }
                            ...
                            if (Has >= Needed) {
                                ...
                                from.SendLocalizedMessage(1060631); // You've completed your binding and received an upgraded version of your scroll!
                                ps.Delete();
                                Delete();
                            } else {
                                ps.Delete();
