...
{
	public class EvalTimer : Timer
	{
        private Aquarium aqua;
    
        public EvalTimer( Aquarium a ) : base( TimeSpan.FromHours( 24 ), TimeSpan.FromHours( 24 ) )
        {
            aqua = a;
        ...
        {
            if( aqua == null || aqua.Deleted )
            {
                Console.WriteLine("Stop akarium Evaluate Timer");
                Stop();
                return;
            }
            aqua.Evaluate();
	...
	{			
		...
		{						
			...
			{						
				...
				// new fish
				if ( OptimalState && Items.Count < MaxItems )
				{
			...
			m_EvaluateDay = true;//!m_EvaluateDay;
