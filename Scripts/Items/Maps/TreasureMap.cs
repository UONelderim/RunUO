...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		public Map ChestMap{
            get{ return m_Map; }
            // 08.09.2014 mortuus: make sure noone changes facet to other than the default Felucca
		...
		{
			/* lvl 0 */ new Type[]{ typeof( HeadlessOne ), typeof( Skeleton ) },
			/* lvl 1 */ new Type[]{ typeof( Mongbat ), typeof( Ratman ), typeof( HeadlessOne ), typeof( Skeleton ), typeof( Zombie ) },
			/* lvl 2 */ new Type[]{ typeof( OrcishMage ), typeof( Gargoyle ), typeof( Gazer ), typeof( HellHound ), typeof( EarthElemental ) },
			/* lvl 3 */ new Type[]{ typeof( Lich ), typeof( OgreLord ), typeof( DreadSpider ), typeof( AirElemental ), typeof( FireElemental ) },
			/* lvl 4 */ new Type[]{ typeof( DreadSpider ), typeof( LichLord ), typeof( CommonDaemon ), typeof( ElderGazer ), typeof( OgreLord ) },
			/* lvl 5 */ new Type[]{ typeof( LichLord ), typeof( CommonDaemon ), typeof( ElderGazer ), typeof( PoisonElemental ), typeof( BloodElemental ) },
			/* lvl 6 */ new Type[]{ typeof( AncientWyrm ), typeof( Balron ), typeof( BloodElemental ), typeof( PoisonElemental ), typeof( Titan ) }
		};
		...
		{
			return false;	// 18.01.2013 mortuus: nie uzywamy Trammel, ale na wszelki wypadek zignorujmy ponizsze wspolrzedne, jako iz tym bardziej nie uzywamy Heaven Island
		...
		{
			m_Level = level;
            // 08.09.2014 mortuus: we only want treasures on Felucca map.
			//m_Map = map;
            m_Map = Map.Felucca;
		...
		private class DigTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				*/
				...
				{
					from.SendMessage( "Musisz miec w plecaku narzedzie do kopania, aby moc wykopac skarb." );
				}
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							switch ( dir )
							{
								case Direction.North:	sDir = "polnoc"; break;
								case Direction.Right:	sDir = "polnocny wschod"; break;
								case Direction.East:	sDir = "wschod"; break;
								case Direction.Down:	sDir = "polowniowy wschod"; break;
								case Direction.South:	sDir = "poludnie"; break;
								case Direction.Left:	sDir = "poludniowy zachod"; break;
								case Direction.West:	sDir = "zachod"; break;
								default:				sDir = "polnocny zachod"; break;
							}

							from.SendAsciiMessage( 0x44, "Sprobuj poszukac skarbu bardziej na {0}.", sDir );
						}
					}
				}
			}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( m_Map == Map.Felucca ? 1041502 : 1041503 ); // "gdzies pod golym niebem" : for somewhere in Trammel

			if ( m_Completed )
			{
				list.Add( 1041507, m_CompletedBy == null ? "kogos" : m_CompletedBy.Name ); // completed by ~1_val~
			}
		}
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( m_Completed )
			{
                from.Send(new MessageLocalizedAffix(Serial, ItemID, MessageType.Label, 0x3B2, 3, 1048030, "", AffixType.Append, String.Format(" odnaleziony przez {0}", m_CompletedBy == null ? "kogos" : m_CompletedBy.Name), ""));
			}
