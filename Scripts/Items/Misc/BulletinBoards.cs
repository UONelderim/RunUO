...
namespace Server.Items
{
	...
	public abstract class BaseBulletinBoard : Item
	{
		...
		{
            m_BoardName = "Tablica Ogloszen";
			Movable = false;
		}

		// Threads will be removed six hours after the last post was made
		private static TimeSpan ThreadDeletionTime = TimeSpan.FromDays( 7.0 );
		...
		public static string FormatTS( TimeSpan ts )
		{
            ...
            if (minutes != 0 && seconds != 0)
                return String.Format("{0} minut{1} i {2} sekund{3}", minutes, minutes == 1 ? "e" : "y", seconds, seconds == 1 ? "e" : "y");
            else if (minutes != 0)
                return String.Format("{0} minut{1}", minutes, minutes == 1 ? "e" : "y");
            else
                return String.Format("{0} sekund{1}", seconds, seconds == 1 ? "e" : "y");
		}
		...
		public static void BBClientRequest( NetState state, PacketReader pvSrc )
		{
			...
			BaseBulletinBoard board = World.FindItem( (Serial)pvSrc.ReadInt32() ) as BaseBulletinBoard;
		...
		{
			BulletinMessage msg = World.FindItem( (Serial)pvSrc.ReadInt32() ) as BulletinMessage;
		...
		{
			BulletinMessage thread = World.FindItem( (Serial)pvSrc.ReadInt32() ) as BulletinMessage;
			...
			if ( board.GetLastPostTime( from, ( thread == null ), ref lastPostTime ) )
			{
				...
				{
                    if (thread == null)
                        from.SendMessage("Musisz poczekac {0} zanim zalozysz nowa wiadomosc.", FormatTS(ThreadCreateTime));
                    else
                        from.SendMessage("Musisz poczekac {0} zanim odpowiedz na istniejaca wiadomosc.", FormatTS(ThreadReplyTime));
