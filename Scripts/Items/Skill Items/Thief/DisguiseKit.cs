...
namespace Server.Items
{
	...
	public class DisguiseGump : Gump
	{
		...
		public DisguiseGump( Mobile from, DisguiseKit kit, bool startAtHair, bool used ) : base( 50, 50 )
		{
			...
			AddHtmlLocalized( 289, 355, 90, 35, 1062520, false, false ); // Pokaz
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( index >= 0 && index < entries.Length )
			{
				...
				if ( m_From is PlayerMobile )
				{
                    ...
                    pm.HueMod = Utility.RandomList(pm.Race.SkinHues);

					if ( hair )
                        pm.SetHairMods(entry.m_ItemID, pm.FacialHairItemID, Utility.RandomList(pm.Race.HairHues));
					else
                        pm.SetHairMods(pm.HairItemID, entry.m_ItemID, Utility.RandomList(pm.Race.HairHues));
				}
