...
namespace Server.Items
{
	...
	public class LeatherNinjaBelt : BaseWaist, IUsesRemaining
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( Fukiya.IsThrowingNinjaThrowable( from ) )
			{
				// You cannot throw another shuriken yet.
				from.SendLocalizedMessage( 1063298 );
			}
			// 09.09.2013 :: mortuus - GM ninja moze uzywac shuriken majac zajete rece
		...
		public void Shoot( Mobile from, Mobile target )
		{
			...
			else if ( from.CanBeHarmful( target ) )
			{
                ...
                // This adds object to shared hasthable with Fukiya (as they cant be used at the same time)
                // Reset item from table is done automatically by timer within Fukiya class
                Fukiya.AddUsing(from, 3.5); 
			}
		}
