	...
	public class Clock : Item
	{
		...
		*/
		...
		public static MoonPhase GetMoonPhase( Map map, int x, int y )
		{
			...
			return ServerTime.GetEnumMoonPhase( true );
		}
		...
		{
			TimeSpan timeSpan = DateTime.Now - ServerTime.WorldStart;
			...
			*/
			...
			hours = ServerTime.Day;
			minutes = ServerTime.TimeUnit * 2;
		}

		public static void GetTime( out string generalNumber, out string exactTime )
		{
			GetTime( null, 0, 0, out generalNumber, out exactTime );
		}

		public static void GetTime( Mobile from, out string generalNumber, out string exactTime )
		{
			GetTime( from.Map, from.X, from.Y, out generalNumber, out exactTime );
		}

		public static void GetTime( Map map, int x, int y, out string descriptiveTime, out string exactTime )
		{
			int hours, minutes, TimeUnit;
			...
			if ( TimeUnit <= 2)
				descriptiveTime = "Slonce dopiero wstalo";
			else if ( TimeUnit <= 8 )
				descriptiveTime = "Jest wczesnie rano";
			else if ( TimeUnit <= 10 )
				descriptiveTime = "Zbliza sie poludnie";
			else if ( TimeUnit <= 12 )
				descriptiveTime = "Jest wczesne popoludnie";
			else if ( TimeUnit <= 18 )
				descriptiveTime = "Jest popoludnie";
			else if ( TimeUnit <= 22 )
				descriptiveTime = "Jest wieczor";
			else if ( TimeUnit <= 25 )
				descriptiveTime = "Jest wczesna noc";
			else if ( TimeUnit <= 27 )
				descriptiveTime = "Jest pozna noc";
			else
				descriptiveTime = "Nadchodzi swit";
			...
			exactTime = String.Format( ", a dokladniej, jest {0} klepsydra", TimeUnit );
		}
