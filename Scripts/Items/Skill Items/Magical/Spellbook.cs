...
namespace Server.Items
{
    ...
    public class Spellbook : Item, ICraftable, ISlayer, IWearableDurability
    {
        // 23.09.2012 :: zombie :: dodanie durability
		public virtual int InitMinHits{ get{ return 80; } }
			...
			if ( Config.ItemDurabilityLostChance > Utility.Random( 100 ) )
			{
				...
				if ( wear > 0 && m_MaxHitPoints > 0 )
				{
                    ...
                    if (m_HitPoints < 5 && Parent is Mobile) {
                        ((Mobile)Parent).LocalOverheadMessage(MessageType.Regular, 0x3B2, 1061187); // Twoja magiczna ksiega sie rozpada!
                    }
        ...
        public Spellbook( ulong content, int itemID ) : base( itemID )
        {
            ...
            // 23.09.2012 :: zombie :: zmiany w wytrzymalosci
        ...
        public override void OnAdded( object parent )
        {
            if ( Core.AOS && parent is Mobile )
            {
                ...
                m_AosSkillBonuses.AddTo( from, true );
		...
		{
            // This is for looted books as these are unblessed due to workaround (See: LootPack.cs, LootPackEntry.Mutate() )
            if (SpellbookType == SpellbookType.Regular) {
                LootType = LootType.Regular;
            } else {
                LootType = LootType.Blessed;
        ...
        public override void Serialize( GenericWriter writer )
        {
            ...
            writer.Write( (byte)1 ); //NegativeAttributes version
            writer.Write( (uint)0 ); //NegativeAttributes names
            writer.WriteEncodedInt( 0 ); //NegativeAttributes count

            writer.Write( (int) m_HitPoints );
            writer.Write( (int) m_MaxHitPoints );

            writer.Write( false ); //_VvVItem
            writer.Write( (Mobile)null ); //_Owner
            writer.Write( (string)null ); //_OwnerName
            writer.Write( (byte)0 ); //m_Quality
            writer.Write( (string)null ); //m_EngravedText
        ...
        public override void Deserialize( GenericReader reader )
        {
            ...
            switch ( version )
            {
                case 6:
                {
                    reader.ReadByte(); //NegativeAttributes version
                    reader.ReadUInt(); //NegativeAttributes names
                    reader.ReadEncodedInt(); //NegativeAttributes count
                    m_MaxHitPoints = reader.ReadInt();
                    m_HitPoints = reader.ReadInt();
                    reader.ReadBool(); //_VvVItem
                    reader.ReadMobile(); //_Owner
                    reader.ReadString(); //_OwnerName
                    reader.ReadByte(); //m_Quality
                    reader.ReadString(); //m_EngravedText
                    goto case 3;
                }
                case 5:
                // 23.09.2012 :: zombie
            ...
            if ( Core.AOS && Parent is Mobile )
                m_AosSkillBonuses.AddTo( (Mobile) Parent, true );
