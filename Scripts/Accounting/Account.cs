...
namespace Server.Accounting
{
	public class Account : IAccount, IComparable, IComparable<Account>
	{
		...
		{
			get { return ( ( m_LastLogin + Config.InactiveDuration ) <= DateTime.Now && AccessLevel == AccessLevel.Player ); }
		}
		...
		private static void EventSink_Login( LoginEventArgs e )
		{
			...
			{
				TimeSpan ts = Config.YoungDuration - acc.TotalGameTime;
		...
		{
			if ( TotalGameTime >= Config.YoungDuration )
				...
				foreach ( XmlElement ele in chars.GetElementsByTagName( "char" ) )
				{
					try
					{
						...
						if ( index >= 0 && index < list.Length )
							list[index] = World.FindMobile( (Serial)serial );
					}
					catch
					{
					}
