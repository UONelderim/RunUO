			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( p != null )
				{
					try
					{
						...
						if ( Config.ValidateLabeling( m_Object ) )
						{
							if ( m_Object is Item )
							{
								((Item)m_Object).ModifiedBy = m_Mobile.Account.Username;
								((Item)m_Object).ModifiedDate = DateTime.Now;
							}
							if ( m_Object is Mobile )
							{
								((Mobile)m_Object).ModifiedBy = m_Mobile.Account.Username;
								((Mobile)m_Object).ModifiedDate = DateTime.Now;
							}			
						}
					}
					catch
