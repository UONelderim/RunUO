...
namespace Server.Gumps
{
	...
	public class NewPlayerVendorOwnerGump : Gump
	{
		...
		public NewPlayerVendorOwnerGump( PlayerVendor vendor ) : base( 50, 200 )
		{
			...
			int currentCharge = vendor.ChargePerRealWorldDay * vendor.UnpaidDays;
			...
			if ( goldHeld < currentCharge )
			{
				AddHtmlLocalized( 40, 35, 260, 20, 1038320, 0x7FFF, false, false ); // wynagrodzenie nalezne sprzedawcy:
				AddLabel( 300, 35, 0x1F, currentCharge.ToString() );

				AddHtmlLocalized( 40, 132, 260, 20, 1062518, 0x7FFF, false, false ); // Ilosc zaleglych dni:
				AddLabel( 300, 132, 0x1F, vendor.UnpaidDays.ToString() );
			}
			else
			{
			...
			AddHtmlLocalized( 40, 106, 260, 20, 1062509, 0x7FFF, false, false ); // Shop Name:
			AddLabel( 140, 105, 0x66D, vendor.ShopName );
			...
			if ( vendor is RentedVendor )
			{
				...
				AddLabel( 38, 156, 0x480, String.Format( "Location rental will expire in {0} day{1} and {2} hour{3}.", days, days != 1 ? "s" : "", hours, hours != 1 ? "s" : "" ) );
			}
			...
			AddLabel( 408, 121, 0x480, "Zabierz zloto" );

			AddButton( 390, 144, 0x15E1, 0x15E5, 7, GumpButtonType.Reply, 0 );
			AddLabel( 408, 141, 0x480, "Zlikwiduj sklep" );
