...
namespace Server.Mobiles
{
	...
	{
		public static int InactiveDaysBeforeDestroy { get {return 1;} }

		private int m_UnpaidDays;
		...
		public PlayerVendor( Mobile owner, BaseHouse house )
		{
			...
			if ( BaseHouse.NewVendorSystem )	// PRAWDA
			{
				m_BankAccount = 0;
				m_HoldGold = 0;
			}
		...
		{
			// podatek 2%:
			int tax = (int) Math.Round(0.02 * price, 0, MidpointRounding.AwayFromZero);

			// podatek minimum 1gp jesli cena > 1gp:
			if (tax == 0 && price>1)
				tax = 1;
			...
			if (tax < 0)
			{
				Console.WriteLine("UWAGA!!! - PlayerVendor.Earn() naliczyl ujemny podatek od sprzedazy!");
				tax = 0;
			}

			m_HoldGold += price - tax;
		}
		...
		{
			get { return m_UnpaidDays; }
			set { m_UnpaidDays = value; }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Locked { get { return (bool)(m_UnpaidDays > 0); } }
		...
		public int ChargePerDay
		{
			...
			{ 
				if ( BaseHouse.NewVendorSystem )	// PRAWDA
				{
					return ChargePerRealWorldDay / 24;
		...
		public int ChargePerRealWorldDay
		{
			get
			{
				...
				{
					if (House != null)
						return 300;	// dzienna oplata za kupca w domku gracza

					return 200;    // dzienna oplata za kupca na targowisku
				}
				else
				{
					return ChargePerDay * 24;
				}
			}
		}
			...
			if ( item is Gold )
			{
				if ( BaseHouse.NewVendorSystem )	// PRAWDA
				{
					if ( this.HoldGold < 1000000 )
					{
						...
						/*
						// Kupiec pobiera zloto za kazdy nalezny dzien jesli tylko jego ilosc wystarczy na jeden dzien.
						// Umozliwia to utrzymywanie nieaktywnego kupca przez wiele dni.
						while ( HoldGold >= ChargePerRealWorldDay && m_UnpaidDays > 0 )
						{
							HoldGold -= ChargePerRealWorldDay;
							m_UnpaidDays--;
						} */
						
						// Kupiec pobiera zloto tylko wowczas, gdy jego ilosc wystarcza na splate calkowitej naleznosci.
						// Zapobiegnie to utrzymywaniu nieaktywnych kupcow dluzej niz 5 dni.
						int unpaidCharge = m_UnpaidDays * ChargePerRealWorldDay;
						if (Locked && this.HoldGold >= unpaidCharge)
						{
							m_UnpaidDays = 0;
							this.HoldGold -= unpaidCharge;
						}
        ...
        // 30.06.2012 :: zombie :: przetlumaczenie komend (przeniesione)
        public override void OnSpeech( SpeechEventArgs e )
        {
            ...
            else if ( Regex.IsMatch( e.Speech, "pokaz", RegexOptions.IgnoreCase ) && WasNamed( e.Speech ) )
            {
                if ( House != null && House.IsBanned( from ) && !IsOwner( from ) )
                {
            ...
            else if ( Regex.IsMatch( e.Speech, "oddaj", RegexOptions.IgnoreCase ) && WasNamed( e.Speech ) )
            {
                if ( IsOwner( from ) )
                {
                    CollectGold( from );
            ...
            else if ( Regex.IsMatch( e.Speech, "status", RegexOptions.IgnoreCase ) && WasNamed( e.Speech ) )
            {
                if ( IsOwner( from ) )
                {
                    SendOwnerGump( from );
            ...
            else if ( Regex.IsMatch( e.Speech, "zwalniam cie", RegexOptions.IgnoreCase ) && WasNamed( e.Speech ) )
            {
                if ( IsOwner( from ) )
                {
                    Dismiss( from );
            ...
            else if ( Regex.IsMatch( e.Speech, "obroc", RegexOptions.IgnoreCase ) && WasNamed( e.Speech ) )
            {
                if ( IsOwner( from ) )
                {
		...
		private class PayTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				m_Vendor.UnpaidDays++;	// pierwszy/kolejny dzien minal

				int pay;
				int totalGold;
				if ( BaseHouse.NewVendorSystem )	// PRAWDA
				{
					pay = m_Vendor.ChargePerRealWorldDay * m_Vendor.UnpaidDays;
				...
				if ( pay > totalGold )
				{
					if( m_Vendor.UnpaidDays > PlayerVendor.InactiveDaysBeforeDestroy )
						m_Vendor.Destroy( !BaseHouse.NewVendorSystem );
				}
				else
				{
					if ( !BaseHouse.NewVendorSystem )	// FALSZ
					{
