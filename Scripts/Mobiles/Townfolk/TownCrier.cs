// 25.06.2012 :: zombie :: integracja z systemem plotek
...
namespace Server.Mobiles
{
	...
	public class TownCrierEntry
	{
        ...
        public TownCrierEntry(string[] lines, DateTime expireTime)
        {
            m_Lines = lines;
	...
	public class TownCrierDurationPrompt : Prompt
	{
		...
		public override void OnResponse( Mobile from, string text )
		{
			...
			{
				from.SendMessage( "Niepoprawna wartosc daty. Sproboj: <godziny:minuty:sekundy>" );
			...
			from.SendMessage( "Dlugosc ustawiona na: {0}", ts );
			from.SendMessage( "Wpisz pierwsza linie wiadomosci krzykacza:" );
	...
	public class TownCrierLinesPrompt : Prompt
	{
		...
		public override void OnResponse( Mobile from, string text )
		{
			...
			from.SendMessage( "Wpisz kolejna linie wiadomosci dla krzykacza, lub nacisnij <ESC> jesli skonczyles." );
		...
		public override void OnCancel( Mobile from )
		{
			...
			if ( m_Lines.Count > 0 )
			{
				...
				from.SendMessage( "Wiadomosc ustawiona." );
			}
			else
			{
				if ( m_Entry != null )
					from.SendMessage( "Wiadomosc usunieta." );
				else
					from.SendLocalizedMessage( 502980 ); // Message entry cancelled.
			}
	...
	public class TownCrierGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			if ( info.ButtonID == 1 )
			{
				m_From.SendMessage( "Wpisz dlugosc wiadomosci. Format: <godziny:minuty:sekundy>" );
			...
			else if ( info.ButtonID > 1 )
			{
				...
				if ( entries != null && index < entries.Count )
				{
					...
					m_From.SendMessage( "Edytujesz wpis #{0}.", index + 1 );
					m_From.SendMessage( "Wpisz pierwsza linie:" );
		...
		public TownCrierGump( Mobile from, ITownCrierEntryList owner ) : base( 50, 50 )
		{
			...
			AddHtml( 8, 8, 300 - 8 - 30, 20, "<basefont color=#FFFFFF><center>WIADOMOSCI KRZYKACZA</center></basefont>", false, false );

			AddButton( 300 - 8 - 30, 8, 0xFAB, 0xFAD, 1, GumpButtonType.Reply, 0 );

			if ( count == 0 )
			{
				AddHtml( 8, 30, 284, 20, "<basefont color=#FFFFFF>Krzykacz nie ma zadnych wiadomosci.</basefont>", false, false );
			}
			else
			{
	...
	public class TownCrier : Mobile, ITownCrierEntryList
	{
        ...
        public TownCrierEntry AddEntry(string[] lines, DateTime expireTime)
        {
            if (m_Entries == null)
                m_Entries = new List<TownCrierEntry>();

            TownCrierEntry tce = new TownCrierEntry(lines, expireTime);
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            if (m_Entries != null)
            {
                ...
                foreach (TownCrierEntry tce in m_Entries)
                {
                    writer.Write((bool)tce.Expired);
                    writer.Write((DateTime)tce.ExpireTime);
                    writer.Write((int)tce.Lines.Length);
                    foreach (string l in tce.Lines)
            ...
            switch (version)
            {
                ...
                case 1:
                    ...
                    for (int i = 0; i < entriesCount; i++)
                    {
                        ...
                        m_linesCount = reader.ReadInt();
                        m_lines = new List<string>();
                        for (int j = 0; j < m_linesCount; j++)
                        {
                            m_lines.Add(reader.ReadString());
                        }
                        if (!m_expired && m_lines.Count > 0)
                        {
                            AddEntry(m_lines.ToArray(), m_expireTime);
                        }
                    }
                    break;
                default:
