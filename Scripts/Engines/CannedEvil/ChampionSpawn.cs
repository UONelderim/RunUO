...
namespace Server.Engines.CannedEvil
{
	public class ChampionSpawn : Item
	{
		...
		private bool m_RandomizeType = true;
		...
		public int MaxKills
		{
			get
			{
				return (int)( ChampionSpawnInfo.GetInfo( m_Type ).SpawnScalar * ( 250 - ( Level * 12 )));
			}
		}
        ...
        {
            switch (Utility.Random(3)) // tylko 3 pierwsze z listy ponizej
            {
                case 0: Type = ChampionSpawnType.VerminHorde; break;    // barracoon
                case 1: Type = ChampionSpawnType.UnholyTerror; break;   // neira
                case 2: Type = ChampionSpawnType.ColdBlood; break;      // rikktor
		...
		public void Respawn()
		{
			...
			while( m_Creatures.Count < ( ChampionSpawnInfo.GetInfo( m_Type ).SpawnScalar * ( 180 - ( GetSubLevel() * 40))) )
			{
				Mobile m = Spawn();
