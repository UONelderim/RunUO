...
namespace Server.Engines.Quests.Haven
{
	...
	public class KillHordeMinionsObjective : QuestObjective
	{
		...
		public override bool IgnoreYoungProtection( Mobile from )
		{
			...
			if ( from is LesserHordeDaemon && from.Map == Map.Trammel && from.X >= 3314 && from.X <= 3814 && from.Y >= 2345 && from.Y <= 3095 ) // Haven island
				return true;
		...
		{
			if ( creature is LesserHordeDaemon && corpse.Map == Map.Trammel && corpse.X >= 3314 && corpse.X <= 3814 && corpse.Y >= 2345 && corpse.Y <= 3095 ) // Haven island
			{
				if ( CurProgress == 0 )
		...
		public override void CheckProgress()
		{
			...
			if ( !m_Ambushed && player.Map == Map.Trammel && player.InRange( new Point3D( 3456, 2558, 50 ), 30 ) )
			{
				...
				if ( Map.Trammel.CanSpawnMobile( x, y, z ) )
				{
					...
					BaseCreature creature = new LesserHordeDaemon();
