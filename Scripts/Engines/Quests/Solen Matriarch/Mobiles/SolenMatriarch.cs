...
namespace Server.Engines.Quests.Matriarch
{
	...
	{
		public override void InitBody() { /* Needs no human body nor skin hue */ }
		...
		public void OnGivenFungi( PlayerMobile player, ZoogiFungus fungi )
		{
			...
			if ( qs != null && qs.RedSolen == this.RedSolen )
			{
				...
				if ( obj != null && !obj.Completed )
				{
					int amount = fungi.Amount / 3;		// oryginalnie 2,  za 200 grzybkow daje 100 proszku

					if ( amount > 40 )					// oryginalnie 100
						amount = 40;					// oryginalnie 100

					if ( amount > 0 )
					{
						if ( amount * 3 >= fungi.Amount )  // oryginalnie 2
							fungi.Delete();
						else
							fungi.Amount -= amount * 3;	// oryginalnie 2
