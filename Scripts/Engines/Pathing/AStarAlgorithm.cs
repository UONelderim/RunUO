...
{
	public class AStarAlgorithm : PathAlgorithm
	{
		...
		public static PathAlgorithm Instance = new AStarAlgorithm();
		...
		public override bool CheckCondition(Mobile m, Map map, Point3D start, Point3D end) {
			return Utility.InRange(start, end, MaxRange);
		}

		public override Direction[] Find(Mobile m, Map map, Point3D start, Point3D end) {
			if (!CheckCondition(m, map, start, end)) return null;
			...
			while( openList.Any()) {
				...
				for (int x = -1; x <= 1; x++) {
					for (int y = -1; y <= 1; y++) {
						...
						if (!CalcMoves.CheckMovement(m, m.Map, curNode.pos, direction, out newZ)) continue;
