...
namespace Server.Engines.BulkOrders
{
	...
	public class SmallSmithBOD : SmallBOD
	{
		public static double[] m_BlacksmithMaterialChances = new double[]
			{
                0.350604457,    // Iron
                0.225254251,    // Dull Copper
                0.150169501,    // Shadow Iron
                0.100113000,    // Copper
                0.066742000,    // Bronze
                0.044494667,    // Gold
                0.029663111,    // Agapite
                0.019775408,    // Verite
                0.013183605     // Valorite
                /* // oryginalnie:
				0.501953125, // None
		...
		public static SmallSmithBOD CreateRandomFor( Mobile m )
		{
			...
			if ( entries.Length > 0 )
			{
				...
				if ( useMaterials && theirSkill >= 70.1 )
				{
					for ( int i = 0; i < 20; ++i )
					{
						BulkMaterialType check = GetRandomMaterial(BulkMaterialType.None, BulkMaterialType.DullCopper, m_BlacksmithMaterialChances );
						double skillReq = 0.0;
		...
		public SmallSmithBOD()
		{
			...
			if ( entries.Length > 0 )
			{
				...
				if ( useMaterials )
					material = GetRandomMaterial(BulkMaterialType.None, BulkMaterialType.DullCopper, m_BlacksmithMaterialChances );
				else
