...
namespace Server.Engines.BulkOrders
{
	...
	public abstract class LargeBOD : Item
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int Level{ get{ return SmallBulkEntry.GetHunterBulkLevel(this); } /*set{ m_Level = value; InvalidateProperties(); }*/ }
		...
		public LargeBOD( int hue, int amountMax, bool requireExeptional, BulkMaterialType material, BulkMaterialType material2, int level, LargeBulkEntry[] entries ) : base( Core.AOS ? 0x2258 : 0x14EF )
		{
			Weight = 1.0;
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
				list.Add( 1060658, "#{0}\t{1}", m_Entries[i].Details.Number, m_Entries[i].Amount ); // ~1_val~: ~2_val~
		}
		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				if ( o is SmallBOD )
				{
					...
					else if (m_Material >= BulkMaterialType.Oak && m_Material <= BulkMaterialType.Frostwood && small.Material != m_Material) 
					{
						from.SendLocalizedMessage(1049798); // Obydwa zlecenia musza byc tego samego typu drewna.
					} 
					else if (m_Material2 >= BulkMaterialType.BowstringLeather && m_Material2 <= BulkMaterialType.BowstringSilk && small.Material2 != m_Material2) 
					{
						from.SendLocalizedMessage(1049799); // Obydwa zlecenia musza byc tego samego typu cieciw.
					} 
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
				m_Entries[i].Serialize( writer, version );
