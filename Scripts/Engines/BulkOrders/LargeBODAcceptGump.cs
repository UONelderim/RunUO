...
namespace Server.Engines.BulkOrders
{
	public class LargeBODAcceptGump : Gump
	{
		...
		public LargeBODAcceptGump( Mobile from, LargeBOD deed ) : base( 50, 50 )
		{
			...
			AddBackground( 25, 10, 430, 270 + (entries.Length * 24), 5054 );

			AddImageTiled( 33, 20, 413, 251 + (entries.Length * 24), 2624 );
			AddAlphaRegion( 33, 20, 413, 251 + (entries.Length * 24) );

			AddImage( 20, 5, 10460 );
			AddImage( 430, 5, 10460 );
			AddImage( 20, 256 + (entries.Length * 24), 10460 );
			AddImage( 430, 256 + (entries.Length * 24), 10460 );
			...
			if ( deed.RequireExceptional || deed.Material != BulkMaterialType.None || deed.Material2 != BulkMaterialType.None)
			{
				...
				if (deed.Material2 != BulkMaterialType.None) {
					AddHtmlLocalized(40, y, 350, 20, SmallBODGump.GetMaterialNumberFor(deed.Material2), 0x7FFF, false, false); // All items must be made with x material.
				}
			} else if (deed.Level > 0)
			{
				AddHtmlLocalized( 40, y, 210, 20, 1063511, 0x7FFF, false, false ); // Poziom trudnosci zlecenia:
				AddLabel( 260, y, 1152, deed.Level.ToString() );
			}

			AddHtmlLocalized( 40, 216 + (entries.Length * 24), 350, 20, 1045139, 0x7FFF, false, false ); // Do you want to accept this order?

			AddButton( 100, 240 + (entries.Length * 24), 4005, 4007, 1, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 135, 240 + (entries.Length * 24), 120, 20, 1006044, 0x7FFF, false, false ); // Ok

			AddButton( 275, 240 + (entries.Length * 24), 4005, 4007, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 310, 240 + (entries.Length * 24), 120, 20, 1011012, 0x7FFF, false, false ); // CANCEL
		}
