...
namespace Server.Engines.BulkOrders
{
	public class SmallBODGump : Gump
	{
		...
		public SmallBODGump( Mobile from, SmallBOD deed ) : base( 25, 25 )
		{
			...
			AddBackground( 50, 10, 455, 290, 5054 );
			AddImageTiled( 58, 20, 438, 275, 2624 );
			AddAlphaRegion( 58, 20, 438, 275 );
			...
			if (deed.RequireExceptional || deed.Material != BulkMaterialType.None || deed.Material2 != BulkMaterialType.None) {
				...
				if (deed.RequireExceptional) {
					AddHtmlLocalized(75, nextY, 420, 20, 1045141, 0x7FFF, false, false); // All items must be exceptional.
					nextY += 24;
				}

				if (deed.Material != BulkMaterialType.None) {
					AddHtmlLocalized(75, nextY, 420, 20, GetMaterialNumberFor(deed.Material), 0x7FFF, false, false); // All items must be made with x material.
					nextY += 24;
				}

				if (deed.Material2 != BulkMaterialType.None) {
					AddHtmlLocalized(75, nextY, 420, 20, GetMaterialNumberFor(deed.Material2), 0x7FFF, false, false); // All items must be made with x material.
					nextY += 24;
				}

			} else if (deed.Level > 0) {
				AddHtmlLocalized(75, 144, 170, 20, 1063511, 0x7FFF, false, false); // Poziom trudnosci zlecenia
				AddLabel(275, 144, 1152, deed.Level.ToString());
			}

			AddButton( 125, 216, 4005, 4007, 2, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 160, 216, 300, 20, 1045154, 0x7FFF, false, false ); // Combine this deed with the item requested.

			AddButton( 125, 240, 4005, 4007, 1, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 160, 240, 120, 20, 1011441, 0x7FFF, false, false ); // EXIT
		}
		...
		public static int GetMaterialNumberFor( BulkMaterialType material )
		{
			if ( material >= BulkMaterialType.DullCopper && material <= BulkMaterialType.Valorite )
				...
				return 1049788 + (int)(material - BulkMaterialType.Oak);
			else if (material >= BulkMaterialType.BowstringLeather && material <= BulkMaterialType.BowstringSilk)
				return 1049794 + (int)(material - BulkMaterialType.BowstringLeather);
