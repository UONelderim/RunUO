...
namespace Server.Engines.BulkOrders
{
	...
	public class SmallFletcherBOD : SmallBOD
	{
		...
		{
			SmallBulkEntry[] entries = SmallBulkEntry.BowFletcher;
			...
			if ( entries.Length > 0 )
			{
				...
				if ( useMaterials && theirSkill >= 70.1 )
				{
					for ( int i = 0; i < 20; ++i )
					{
						BulkMaterialType check = GetRandomMaterial(BulkMaterialType.None, BulkMaterialType.Oak, m_BowFletchingMaterialChances );
						double skillReq = 0.0;

						switch ( check )
						{
							case BulkMaterialType.Oak: skillReq = 65.0; break;
							case BulkMaterialType.Ash: skillReq = 75.0; break;
							case BulkMaterialType.Yew: skillReq = 85.0; break;
				...
				if ( validEntries.Count > 0 )
				{
					...
					return new SmallFletcherBOD(entry, material, material2, amountMax, reqExceptional) ;
				}
			}
		...
		private SmallFletcherBOD( SmallBulkEntry entry, BulkMaterialType material, BulkMaterialType material2, int amountMax, bool reqExceptional )
		{
			this.Hue = 1425;
		...
		public SmallFletcherBOD()
		{
			...
			if ( entries.Length > 0 )
			{
				...
				if ( useMaterials )
					material = GetRandomMaterial(BulkMaterialType.None, BulkMaterialType.Oak, m_BowFletchingMaterialChances);
				else
		...
		public SmallFletcherBOD( int amountCur, int amountMax, Type type, int number, int graphic, bool reqExceptional, BulkMaterialType mat, BulkMaterialType mat2)
		{
			this.Hue = 1425;
			this.AmountMax = amountMax;
