...
namespace Server.Engines.Harvest
{
	public abstract class HarvestSystem
	{
		...
		public virtual bool CheckResources( Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, bool timed )
		{
			...
			bool available = ( bank != null && bank.Current > 0 && bank.Current >= def.ConsumedPerHarvestMin );
		...
		public virtual object GetLock( Mobile from, Item tool, HarvestDefinition def, object toHarvest )
		{
            ...
            // 19.09.2012 :: zombie :: zablokowanie multi harvestingu: return tool ->return typeof( HarvestSystem )
            // szczaw :: 2013.01.17 :: harvest lub craft, nie obydwie akcje na raz... typeof(HarvestSystem) -> typeof(HarvestOrCraftlock)
		...
		public virtual void FinishHarvesting( Mobile from, Item tool, HarvestDefinition def, object toHarvest, object locked )
		{
			...
			int amount;
			if ( def.ConsumedPerHarvestMin >= def.ConsumedPerHarvestMax )	// sanity
				amount = def.ConsumedPerHarvestMin;
			else
			{
				double skill_0_100 = Math.Max( 0.0, Math.Min( skillValue, 100.0 ) );
				amount = def.ConsumedPerHarvestMin + (int)Math.Round( skill_0_100 / 100.0 * (def.ConsumedPerHarvestMax-def.ConsumedPerHarvestMin), 0, MidpointRounding.AwayFromZero );
			}
