		...
		protected override void OnTarget( Mobile from, object targeted )
		{
			if ( m_System is Mining && targeted is StaticTarget )
			{
				...
				if ( itemID == 0xED3 || itemID == 0xEDF || itemID == 0xEE0 || itemID == 0xEE1 || itemID == 0xEE2 || itemID == 0xEE8 )
				{
					...
					if ( player != null )
					{
		                ...
		                IPooledEnumerable eable = player.Map.GetItemsInRange(staticTarget.Location, 1);
		                
		                GraveNode grave = null;
		                
						foreach(Item item in eable)
						{
							if (item is GraveNode)
							{
								grave = (GraveNode) item;
								break;
							}
						}
            ...
            else if ( m_System is Lumberjacking && targeted is IAxe && m_Tool is BaseAxe )
			{
				...
				if ( !item.IsChildOf( from.Backpack ) )
					from.SendLocalizedMessage( 1062334 ); // Musisz miec przedmiot w plecaku, zeby go uzyc.
