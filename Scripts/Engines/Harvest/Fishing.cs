...
namespace Server.Engines.Harvest
{
	public class Fishing : HarvestSystem
	{
		...
		private Fishing()
		{
			...
			fish.ConsumedPerHarvestMin = 1;
			fish.ConsumedPerHarvestMax = 1;
			...
			fish.NoResourcesMessage = 503172; // Wyglada na to, ze ryby w tym miejscu nie biora.
			fish.FailMessage = 503171; // Wedkowales przez chwile, ale nic nie wylowiles.
			fish.TimedOutOfRangeMessage = 500976; // Jestes za daleko.
			fish.OutOfRangeMessage = 500976; // Jestes za daleko.
			fish.PackFullMessage = 503176; // Twoj plecak jest przepelniony, nie zmiesci wiecej ryb.
			fish.ToolBrokeMessage = 503174; // Zlamales wedke!
            ...
            {
				new BonusHarvestResource( 0, 85, null, null ),
                new BonusHarvestResource( 0, 15, 1070061, typeof( BlackPearl ) )         
            };
		...
		private static MutateEntry[] m_MutateTable = new MutateEntry[]
			{
                // !!! uwaga: kolejnosc gra role. Przedmioty o malej szansie dajemy jako pierwsze,
                //            aby zminimalizowac zaleznosc prawdopodobienstwa.

                new MutateEntry(  80.0,  80.0,  4080.0,  true, typeof( WhitePearl ) ),  // 20/4000 = 0.5 %
                ...
                new MutateEntry(  80.0,  80.0,  1080.0,  true, typeof( Salt ) ),  // 20/1000 = 2%
		...
		public override Item Construct( Type type, Mobile from )
		{
			...
			{
				int level = 1;	// 13.01.2013 - mortuus: brak HeavenIsland i Trammel na mapie Nelderim.
				/* if ( from is PlayerMobile && ((PlayerMobile)from).Young && from.Map == Map.Trammel && TreasureMap.IsInHavenIsland( from ) )
					level = 0;
				else
					level = 1; */

                // skarby na Nel wystepuja tylko na Felucca:
			...
			if ( pack != null )
			{
				...
				for ( int i = 0; i < messages.Count; ++i )
				{
					...
					if ( from.Map == sos.TargetMap && from.InRange( sos.TargetLocation, 60 ) )
					{
						...
						TreasureMapChest.Fill( chest, Math.Max( 2, Math.Min( 6, sos.Level ) ) );	// 18.01.2013 - mortuus: poprawka buga RunUO
		...
		public override bool Give( Mobile m, Item item, bool placeAtFeet )
		{
			if ( item is TreasureMap || item is MessageInABottle || item is SpecialFishingNet )
			{
				...
				m.SendLocalizedMessage( 503170 ); // Ho ho! To nie wyglada na rybe!
		...
		public override void SendSuccessTo( Mobile from, Item item, HarvestResource resource )
		{
			if ( item is BigFish )
			{
				from.SendLocalizedMessage( 1042635 ); // Twoja siec wygina sie pod wplywem duzej ryby z glebin!

				((BigFish)item).Fisher = from;
			}
			else if ( item is WoodenChest || item is MetalGoldenChest )
			{
				from.SendLocalizedMessage( 503175 ); // Z glebin oceanu wyciagasz ciezko skrzynie!
			}
			else
			{
				int number;
				...
				{
					number = 1008124;	// Wyciagasz przedmiot: 
					name = "lawica magicznych rybek";
				}
				...
				{
					number = 1008125;	// Wraz z potworem wyciagnales przedmiot: 
					name = "przemoczony fragment pergaminu";
				}
				else if ( item is MessageInABottle )
				{
					number = 1008125;	// Wraz z potworem wyciagnales przedmiot: 
					name = "butelka z wiadomoscia w srodku";
				}
				else if ( item is SpecialFishingNet )
				{
					number = 1008125;	// Wraz z potworem wyciagnales przedmiot: 
					name = "specjalna siec do lowienia"; // TODO: this is just a guess--what should it really be named?
				}
				else
				{
					number = 1043297;
					/* // usuniecie angielskich przedrostkow:
		...
		public override bool BeginHarvesting( Mobile from, Item tool )
		{
			...
			from.SendLocalizedMessage( 500974 ); // Wskaz miejsce, gdzie chcesz lowic:
			return true;
		}
		...
		public override bool CheckHarvest( Mobile from, Item tool, HarvestDefinition def, object toHarvest )
		{
			...
			if ( from.Mounted )
			{
				from.SendLocalizedMessage( 500971 ); // Nie mozesz lowic z konia!
				return false;
			}
