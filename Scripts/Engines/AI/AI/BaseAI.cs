    ...
    public abstract class BaseAI
    {
        ...
        public virtual bool Think()
        {
            ...
            switch( Action )
            {
                ...
                // 25.06.2012 :: zombie :: Proba zmiany celu
                if ( DateTime.Now > m_NextTargetChange && !m_Mobile.Controlled && !m_Mobile.Summoned && m_Mobile.Combatant != null )
                {
                    ...
                    if ( m_Mobile.Combatant is BaseCreature )
                    {
                        ...
                        if ( owner != null )
                        {
                            if ( Utility.RandomDouble() < m_Mobile.AttackMasterChance && m_Mobile.GetDistanceToSqrt( owner ) < 3 && m_Mobile.InLOS( owner ) )
                                newTarget = owner;
                        }
                    }
        ...
        // 24.06.2012 :: zombie
        public virtual bool ChangeCombatant( Mobile m, Mobile c )
        {
            double chance = 1.0;
            ...
            try
            {
                ...
                // || m.Int < 50 ||
                if ( chance <= 0 || m.Hits < m.HitsMax * 0.1 || c.Hits < m.HitsMax * 0.4 )
                    return false;

                //chance *= 1 - ( ( m.Hits - ( m.HitsMax * 0.1 ) ) / ( m.HitsMax * 0.9 ) );
                //chance *= ( c.Hits - ( c.HitsMax * 0.4 ) ) / ( c.HitsMax * 0.45 );
                //chance *= ( m.Int - 50 ) * 0.01;
            }
            catch ( Exception exc )
            {
        ...
        {
            //TODO: osobna klasa dla tych WarMode itd. + tablica gdzie index == ActionType
            switch( Action )
            {
                case ActionType.Wander:
        ...
        public virtual void OnCurrentOrderChanged()
        {
            ...
            // TODO: klasa + tablica gdzie index == OrderType
        ...
        // 23.06.2012 :: zombie :: komendy dla zwierzakow/przywolancow (przeniesione)
        public virtual void OnSpeech( SpeechEventArgs e )
        {
            if(e.Mobile.Alive && e.Mobile.InRange(m_Mobile.Location, 3) && m_Mobile.IsHumanInTown())
            {
                ...
                else if(e.HasKeyword(0x9E) && WasNamed(e.Speech)) // *time*
                {
                    ...
                    else
                    {
                        ...
                        m_Mobile.SendMessage(generalNumber + exactTime);
                    }
                }
            ...
            if(m_Mobile.Controlled && m_Mobile.Commandable)
            {
                ...
                if(!e.Handled && e.Mobile.Alive && (isOwner || isFriend))
                {
                    ...
                    else if((WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile)) && (Regex.IsMatch(e.Speech, "idz za mna", RegexOptions.IgnoreCase) || Regex.IsMatch(e.Speech, "idz za mna", RegexOptions.IgnoreCase)))
                    {
                        m_Mobile.ControlTarget = e.Mobile;
                        m_Mobile.ControlOrder = OrderType.Follow;
                        return;
                    ...
                    else if((WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile)) && (Regex.IsMatch(e.Speech, "stoj", RegexOptions.IgnoreCase) || Regex.IsMatch(e.Speech, "stoj", RegexOptions.IgnoreCase)))
                    {
                    ...
                    else if((WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile)) && (Regex.IsMatch(e.Speech, "patroluj", RegexOptions.IgnoreCase)))
                    {
                        if(!isOwner)
                            return;
                    ...
                    else if((WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile)) && (Regex.IsMatch(e.Speech, "przyjaciel", RegexOptions.IgnoreCase)))
                    {
                        if(!isOwner)
                            return;
        ...
        {
            // niech wypuszczane owce nie znikaja:
            if (m_Mobile is Sheep)
                return DoOrderRelease(false);

            return DoOrderRelease(true);
        }

        public virtual bool DoOrderRelease(bool toBeRemoved)
        {
            Console.WriteLine("ROZKAZ WYPUSZCZENIA PETA (DoOrderRelease): OwnerSerial({0}) OwnerName({1}) Serial({2}) Name({3}) ", m_Mobile.ControlMaster != null ? m_Mobile.ControlMaster.Serial.ToString() : "-", m_Mobile.ControlMaster != null ? m_Mobile.ControlMaster.Name : "-", m_Mobile.Serial, m_Mobile.Name);
           ...
           if (toBeRemoved)
            ...
            // 2013.01.18 :: szczaw :: zwierzeta nie upuszczaja plecaka, bo i po co? 
        ...
        public double TransformMoveDelay( double delay )
        {
            ...
            // Przyspieszenie martwych wiernych stworzen
        ...
        #region Timer odliczajacy czas nadania graczowi statusu Kryminalisty w regionie nietolerancji
        ...
        public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe )
        {
            ...
            // 24.06.2012 :: zombie
            if ( m_Mobile.Meditating && ( m_Mobile.Combatant != null && m_Mobile.Combatant.Paralyzed ) )
                return false;
            // zombie
            ...
            if ( map != null )
            {
                ...
                foreach( Mobile m in eable )
                {
                    ...
                    if ( !( m_Mobile is BaseNelderimGuard ) && m is PlayerMobile && ( (PlayerMobile)m ).HonorActive && !( m_Mobile.Combatant == m ))
                        if(!(m_Mobile.IgnoreHonor && (m.Aggressed.Count != 0 || m.Aggressors.Count != 0))) //Mobs with ignore honor won't attack until you attacked anything
                            continue;

                    // Ignore player under Mask of Death effect
					if ( !( m_Mobile is BaseNelderimGuard ) && m is PlayerMobile && m.MaskOfDeathEffect && !( m_Mobile.Combatant == m ))
                        continue;
        ...
        {
            if (m.Race.Equals(Drow.Instance) || TownDatabase.IsCitizenOfGivenTown(m, Towns.LDelmah))
                return true;
        ...
        public virtual bool IsInHarmfulField
        {
            get
            {
        ...
        protected void OnCombatantFled( Mobile combatant )
        {
        ...
        protected void OnGuardActionAttack( Mobile combatant )
        {
            if ( !( m_Mobile is BaseNelderimGuard ) )
                return;
            ...
            if ( m_Mobile.FocusMob.Player && ((BaseNelderimGuard)m_Mobile).IsHuman && ( rand = Utility.Random( 0, 10 ) ) > 5 )
            {
                ...
                {
                    case 6:
                       msg =  String.Format( "Ha! {0}! Ty psi pomiocie! Dopadne Cie i ukarze w imie Sprawiedliwosci!", m_Mobile.FocusMob.Name );
                    break;
                    case 7: case 8: 
                       msg =  String.Format( "{0}! Stoj!", m_Mobile.FocusMob.Name );
                    break;
                    default:
                       msg =  String.Format(  "Stoj!" );
                    break;
                }
        ...
        protected void OnGuardActionWarden()
        {
            ...
            if ( map != null )
            {
                IPooledEnumerable eable = map.GetMobilesInRange( m_Mobile.Location, 6 );
                bool say = false;
                ...
                foreach ( Mobile m in eable )
                {
                    ...
                    {
                        say = true;
                        // Console.WriteLine( "Zatwierdzam {0} jako cel dla plotek!", m.Name );
                        break;
                    }
                }

                if ( say )
                    m_Mobile.AnnounceRandomRumor( PriorityLevel.High );
