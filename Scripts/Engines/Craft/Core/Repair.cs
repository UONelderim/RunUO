...
namespace Server.Engines.Craft
{
    public class Repair
    {
        ...
        public static bool Simulate(Mobile from, object armor)
        {
            return Simulate(from, armor, 120.0);
        }
        public static bool Simulate(Mobile from, object armor, double level)
        {
            return InternalTarget.Simulate(from, armor, level);
        }
        ...
        private class InternalTarget : Target
        {
            ...
            public static bool Simulate(Mobile from, object armor, double level )
            {
                // find appropriate crafting system
                ...
                {
                    CraftItem item = sys.CraftItems.SearchFor( armor.GetType() );
                    if ( item != null )
                    {
                        system = sys;
                        break;
                ...
                if( system == null )
                {
                    Console.WriteLine("no craft system repairs this! {0}", armor.GetType());
                    return false;
                }

                // create a deed
                RepairDeed.RepairSkillType skill = RepairDeed.RepairSkillType.Carpentry;    // irrelevant init
                bool systemHasDeed = true;
                if( system is DefTailoring )
                    skill = RepairDeed.RepairSkillType.Tailoring;

                else if( system is DefBlacksmithy )
                    skill = RepairDeed.RepairSkillType.Smithing;

                else if( system is DefTinkering )
                    skill = RepairDeed.RepairSkillType.Tinkering;

                else if( system is DefBowFletching )
                    skill = RepairDeed.RepairSkillType.Fletching;

                else if( system is DefCarpentry )
                    skill = RepairDeed.RepairSkillType.Carpentry;

                else if( system is DefInscription )
                    skill = RepairDeed.RepairSkillType.Inscription;

                if( !systemHasDeed )
                {
                    Console.WriteLine("this craft system has no repair deeds! {0}", system);
                    return false;
                }
                
                RepairDeed deed = new RepairDeed( skill, level );
                from.AddToBackpack(deed);

                InternalTarget target = new InternalTarget(system, deed);
                target.OnTarget(from, armor);

                return true;
            }

            //public static void Do(Mobile from, object targeted, CraftSystem m_CraftSystem, BaseTool m_Tool, RepairDeed m_Deed)
            protected override void OnTarget( Mobile from, object targeted)
            {
                ...
                // 25.09.2012 :: zombie :: naprawa jeweli i spellbookow
                if ( m_CraftSystem is DefTinkering && targeted is BaseJewel )
                {
                    ...
                    else
                    {
                        ...
                        if ( CheckRepairDifficulty( from, skill, jewel.HitPoints, jewel.MaxHitPoints ) )
                        {
                            ...
                            {
                                jewel.ModifiedBy = from.Account.Username;
                                jewel.ModifiedDate = DateTime.Now;
                            }
                        }
                        else
                        {
                ...
                else if ( m_CraftSystem is DefInscription && targeted is Spellbook )
                {
                    SkillName skill = m_CraftSystem.MainSkill;
                    ...
                    else if ( book.MaxHitPoints <= 0 || book.HitPoints == book.MaxHitPoints )
                    {
                        number = 1044281; // That item is in full repair
                    }
                    else if ( book.MaxHitPoints <= toWeaken )
                    {
                        number = 1044278; // That item has been repaired many times, and will break if repairs are attempted again.
                    }
                    else
                    {
                        if ( CheckWeaken( from, skill, book.HitPoints, book.MaxHitPoints ) )
                        {
                            book.MaxHitPoints -= toWeaken;
                            book.HitPoints = Math.Max( 0, book.HitPoints - toWeaken );
                        }

                        if ( CheckRepairDifficulty( from, skill, book.HitPoints, book.MaxHitPoints ) )
                        {
                            number = 1044279; // You repair the item.
                            m_CraftSystem.PlayCraftEffect( from );
                            book.HitPoints = book.MaxHitPoints;
                            if ( from.AccessLevel > AccessLevel.Player )
                            {
                                book.ModifiedBy = from.Account.Username;
                ...
                else if ( targeted is BaseWeapon )
                {
                    ...
                    else
                    {
                        ...
                        if ( CheckRepairDifficulty( from, skill, weapon.HitPoints, weapon.MaxHitPoints ) )
                        {
                            ...
                            if ( from.AccessLevel > AccessLevel.Player )
                            {
                                weapon.ModifiedBy = from.Account.Username;
                                weapon.ModifiedDate = DateTime.Now;
                            }    
                        }
                        else
                        {
                ...
                else if ( targeted is BaseArmor )
                {
                    ...
                    else
                    {
                        ...
                        if ( CheckRepairDifficulty( from, skill, armor.HitPoints, armor.MaxHitPoints ) )
                        {
                            ...
                            if ( from.AccessLevel > AccessLevel.Player )
                            {
                                armor.ModifiedBy = from.Account.Username;
                                armor.ModifiedDate = DateTime.Now;
                            }
                        }
                        else
                        {
                ...
                else if( !usingDeed && targeted is BlankScroll )
                {
                    ...
                    if( from.Skills[skill].Value >= 50.0 )
                    {
                        ...
                        if ( from.AccessLevel > AccessLevel.Player )
                        {
                            deed.ModifiedBy = from.Account.Username;
                            deed.ModifiedDate = DateTime.Now;
                        }
                ...
                else if (!usingDeed && targeted is RepairDeed)
                {
                    ...
                    if (deed.RepairSkill != RepairDeed.GetTypeFor(m_CraftSystem))
                    {
                        ...
                        from.SendMessage("Wskazany zwój dotyczy innego rzemiosla!");
                    }
                    ...
                    else if (deed.UsesRemaining >= deed.UsesMax)
                    {
                        ...
                        from.SendMessage("Ten zwój jest juz w pelni naladowany. Uzyj czystego zwoju.");
                    }
