	...
	public class CraftGump : Gump
	{
		...
		private CraftGump( Mobile from, CraftSystem craftSystem, BaseTool tool, object notice, CraftPage page ) : base( 40, 40 )
		{
			...
			if ( craftSystem.CraftSubRes.Init )
			{
                ...
                {
                    Item[] items = from.Backpack.FindItemsByType( Server.Engines.Craft.CraftItem.GetAlternativeTypes(resourceType), true );
				...
				if ( nameNumber > 0 )
                    ...
                    AddLabel( 50, 362, LabelHue, String.Format( "{0} ({1})", nameString, resourceCount ) );
			}
			// ****************************************
            ...
            // zombie - 17-06-2012 - dodanie argumentu from
        ...
        // zombie - 17-06-2012 - poprawione zliczanie surowcow w gumpie (z RunUO 2.2)
        public void CreateResList(bool opt, Mobile from)
        {
            ...
            for (int i = 0; i < res.Count; ++i)
            {
                ...
                if (from.Backpack != null)
                {
                    Item[] items = from.Backpack.FindItemsByType(Server.Engines.Craft.CraftItem.GetAlternativeTypes(subResource.ItemType), true);
		...
		public void CraftItem( CraftItem item )
		{
			...
			else
			{
                ...
                //if ( item.UseSubRes2 )
				//    m_CraftSystem.CreateItem( m_From, item.ItemType, type2, type, m_Tool, item );
                //else
			...
			switch ( type )
			{
				...
				case 5: // Resource selected
				{
					if ( m_Page == CraftPage.PickResource && index >= 0 && index < system.CraftSubRes.Count )
					{
						...
						if ( !system.ResourcesUnlocked(m_From, res.ItemType, null) ) // czy postac umie wykorzystywac danu material?
						{
							m_From.SendGump( new CraftGump( m_From, system, m_Tool, 1032588 ) );
						}
						else if ( m_From.Skills[system.MainSkill].Base < res.RequiredSkill )
						{
					...
					else if ( m_Page == CraftPage.PickResource2 && index >= 0 && index < system.CraftSubRes2.Count )
					{
						...
						if ( !system.ResourcesUnlocked(m_From, null, res.ItemType) ) // czy postac umie wykorzystywac danu material?
						{
							m_From.SendGump( new CraftGump( m_From, system, m_Tool, 1032588 ) );
						}
						else if ( m_From.Skills[system.MainSkill].Base < res.RequiredSkill )
				...
				case 6: // Misc. buttons
				{
					switch ( index )
					{
						...
						{
                            if (system.Recycle)
                                system.RecycleHelper.Do(m_From, system, m_Tool);
