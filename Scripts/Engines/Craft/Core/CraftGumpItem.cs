...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
		...
		public void DrawSkill()
		{
            ...
            // Rozpatrzec sens UseSubRes2 w tym przypadku (zmieniona kolejnosc argumentow).
            // Narazie bez znaczenia bo GetSuccessChance nie uzywa argumentu typeRes, ale ogolnie nie powinno raczej zamieniac kolejnosci.

			CraftSubResCol res = m_CraftSystem.CraftSubRes; // ( m_CraftItem.UseSubRes2 ? m_CraftSystem.CraftSubRes2 : m_CraftSystem.CraftSubRes );
			int resIndex = -1;

			CraftSubResCol resOpposite = m_CraftSystem.CraftSubRes2; //( m_CraftItem.UseSubRes2 ? m_CraftSystem.CraftSubRes : m_CraftSystem.CraftSubRes2 );
			int resOppositeIndex = -1;

			CraftContext context = m_CraftSystem.GetContext( m_From );

			if ( context != null )
            {
				resIndex = context.LastResourceIndex; //( m_CraftItem.UseSubRes2 ? context.LastResourceIndex2 : context.LastResourceIndex );
                resOppositeIndex = context.LastResourceIndex2; //( m_CraftItem.UseSubRes2 ? context.LastResourceIndex : context.LastResourceIndex2 );
            }
            Type typeRes = resIndex > -1 ? res.GetAt( resIndex ).ItemType : null;
            Type typeRes2 = resOppositeIndex > -1 ? resOpposite.GetAt( resOppositeIndex ).ItemType : null;

			bool allRequiredSkills = true;
			double chance = m_CraftItem.GetSuccessChance( m_From, typeRes, typeRes2, m_CraftSystem, false, ref allRequiredSkills );
		...
		public void DrawRessource()
		{
            ...
            CraftSubResCol res2 = m_CraftSystem.CraftSubRes2;
			int resIndex = -1;
            int resIndex2 = -1;
			...
			if ( context != null )
            {
                ...
                resIndex2 = context.LastResourceIndex2;
            }
			...
			for ( int i = 0; i < m_CraftItem.Ressources.Count - (cropScroll ? 1 : 0) && i < 4; i++ )
			{
				...
				else if ( type == res2.ResType && resIndex2 > -1 )
				{
					CraftSubRes subResource2 = res2.GetAt( resIndex2 );

					type = subResource2.ItemType;

					nameString = subResource2.NameString;
					nameNumber = subResource2.GenericNameNumber;

					if ( nameNumber <= 0 )
						nameNumber = subResource2.NameNumber;
				}
				// ******************
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			else // Make Button
			{
				...
				else
				{
					...
					if ( context != null )
					{
                        CraftSubResCol res = m_CraftSystem.CraftSubRes; // lista kolorowych materialow typu pierwszego
                        int resIndex = context.LastResourceIndex;   // wybrana pozycja w menu materialow typu pierwszego
                        if ( resIndex >= 0 && resIndex < res.Count )
                            type = res.GetAt( resIndex ).ItemType;

                        CraftSubResCol res2 = m_CraftSystem.CraftSubRes2; // lista kolorowych materialow typu drugiego
                        int resIndex2 = context.LastResourceIndex2;   // wybrana pozycja w menu materialow typu drugiego
                        if ( resIndex2 >= 0 && resIndex2 < res2.Count )
                            type2 = res2.GetAt( resIndex2 ).ItemType;
					}

                    // Material bonus is determined by the first resource type, not the second.
                    //if ( m_CraftItem.UseSubRes2 )
					//    m_CraftSystem.CreateItem( m_From, m_CraftItem.ItemType, type2, type, m_Tool, m_CraftItem );
                    //else
                    //    m_CraftSystem.CreateItem( m_From, m_CraftItem.ItemType, type, type2, m_Tool, m_CraftItem );

                    // mortuus:
                    // Priorytetyzowanie surowca wg zmiennej CraftItem.UseSubRes2 zostalo przeniesione do innych metod
                    // ( Enhance.Invoke(), CraftItem.CompleteCraft() )
                    // Bonusy od obu materialow sa wazne, ale Hue definiuje tylko pierwszy material.
                    m_CraftSystem.CreateItem( m_From, m_CraftItem.ItemType, type, type2, m_Tool, m_CraftItem );
				}
			}
		}
