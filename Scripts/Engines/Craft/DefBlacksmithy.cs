...
namespace Server.Engines.Craft
{
	public class DefBlacksmithy : CraftSystem
	{
		...
		{
			// 30.10.2012 :: zombie :: dodanie animacji
			if ( from.Body.Type == BodyType.Human && !from.Mounted )
				from.Animate( 9, 5, 1, true, false, 0 );
			new InternalTimer( from ).Start();
            // zombie

			//from.PlaySound( 0x2A );	dzwiek jest w timerze
		}
		...
		public override void InitCraftList()
		{
			...
			*/
			...
			if ( Core.AOS ) // exact pre-aos functionality unknown
				AddCraft( typeof( DragonBardingDeed ), 1011078, 1053012, 72.5, 122.5, typeof( IronIngot ), 1044036, 450, 1044037 );
				AddCraft( typeof( WarHorseBardingDeed ), 1011078, "Projekt Pancerza na Konia bojowego", 72.5, 122.5, typeof( IronIngot ), 1044036, 450, 1044037 );
			...
			if( Core.SE )
			{
				...
				SetNeededExpansion( index, Expansion.SE );
				index = AddCraft( typeof( StarozytneRekawice ), 1011078, "starożytne płytowe rękawice", 70.0, 100.1, typeof( IronIngot ), 1044036, 13, 1044037 );
			...
			if( Core.SE )
			{
				...
				SetNeededExpansion( index, Expansion.SE );
				index = AddCraft( typeof( Circlet ), 1011079, "dekoracyjna obręcz", 62.1, 112.1, typeof( IronIngot ), 1044036, 6, 1044037 );
					

					index = AddCraft( typeof( RoyalCirclet ), 1011079, "starożytna dekoracyjna obręcz" , 70.0, 120.0, typeof( IronIngot ), 1044036, 6, 1044037 );
					index = AddCraft( typeof( GemmedCirclet ), 1011079, "dekoracyjna obręcz wysadzana kamieniami", 75.0, 125.0, typeof( IronIngot ), 1044036, 6, 1044037 );
			...
			if( Core.SE )
			{
				...
				index = AddCraft( typeof( RadiantScimitar ), 1011081, 1031571, 75.0, 125.0, typeof( IronIngot ), 1044036, 15, 1044037 );
					...
					index = AddCraft( typeof( GreatSword ), 1011081, "ogromny miecz", 78.0, 122.0, typeof( IronIngot ), 1044036, 30, 1044037 );
					
					index = AddCraft( typeof( RuneBlade ), 1011081, "ostrza zdobione runami" , 70.0, 120.0, typeof( IronIngot ), 1044036, 15, 1044037 );
					

					index = AddCraft( typeof( ElvenMachete ), 1011081, "maczeta", 70.0, 120.0, typeof( IronIngot ), 1044036, 14, 1044037 );
			...
			index = AddCraft( typeof( DiamondMace ), 1011084, "diamentowy młot" , 70.0, 120.0, typeof( IronIngot ), 1044036, 20, 1044037 );
            ...
            // 1053114	Smocza zbroja
            // 1029795	smocze rekawice

            // 1060883	Smocze luski
            // 1060884	Brakuje ci odpowiednich lusek do zrobienia tego.

            // 1044036	Sztaby
            // 1044037	Brakuje ci odpowiednich sztab do zrobienia tego.

			index = AddCraft( typeof( DragonGloves ), 1053114, 1029795, 78.9, 118.9, typeof( RedScales ), 1060883, 16, 1060884 );
			AddRes( index, typeof(IronIngot), 1044036, 12, 1044037 );
            SetUseSubRes2( index, true );

			index = AddCraft( typeof( DragonHelm ), 1053114, 1029797, 82.6, 122.6, typeof( RedScales ), 1060883, 20, 1060884 );
			AddRes( index, typeof(IronIngot), 1044036, 15, 1044037 );
            SetUseSubRes2( index, true );

			index = AddCraft( typeof( DragonLegs ), 1053114, 1029799, 88.8, 128.8, typeof( RedScales ), 1060883, 28, 1060884 );
			AddRes( index, typeof(IronIngot), 1044036, 20, 1044037 );
            SetUseSubRes2( index, true );

			index = AddCraft( typeof( DragonArms ), 1053114, 1029815, 86.3, 126.3, typeof( RedScales ), 1060883, 24, 1060884 );
			AddRes( index, typeof(IronIngot), 1044036, 18, 1044037 );
            SetUseSubRes2( index, true );

			index = AddCraft( typeof( DragonChest ), 1053114, 1029793, 90.0, 135.0, typeof( RedScales ), 1060883, 36, 1060884 );
			...
			AddSubRes( typeof( IronIngot ),			1044022, 00.0, 1044036, 1044267 );  // 1044267  Musisz byc obok kowadla i paleniska, zeby wykuwac przedmioty.
			AddSubRes( typeof( DullCopperIngot ),	1044023, 65.0, 1044036, 1044268 );  // 1044036  Sztaby
			AddSubRes( typeof( ShadowIronIngot ),	1044024, 70.0, 1044036, 1044268 );  // 1044268  Nie wiesz jak wykuc cos z tego metalu.
			...
			AddSubRes2( typeof( RedScales ),		1060875, 0.0, 1053137, 1044268 );   // 1053137	Luski smoka
			AddSubRes2( typeof( YellowScales ),		1060876, 0.0, 1053137, 1044268 );   // 1044268	Nie wiesz jak wykuc cos z tego metalu.
