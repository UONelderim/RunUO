...
namespace Server.Spells
{
	public class SpellInfo
	{
        ...
        // 05.07.2012 :: zombie :: dodanie konstuktora z argumentem amounts
		public SpellInfo( string name, string mantra, int action, int handEffect, int[] amounts, params Type[] regs ) : this( name, mantra, action, handEffect, handEffect, true, amounts, regs )
		{
		}
        // zombie             
        ...
        // 05.07.2012 :: zombie :: dodanie argumentu amounts
        public SpellInfo( string name, string mantra, int action, int leftHandEffect, int rightHandEffect, bool allowTown, params Type[] regs ) : this( name, mantra, action, leftHandEffect, rightHandEffect, allowTown, null, regs )
        {
        }

		public SpellInfo( string name, string mantra, int action, int leftHandEffect, int rightHandEffect, bool allowTown, int[] amounts, params Type[] regs )
        // zombie
		{
			m_Name = name;
			m_Mantra = mantra;
            ...
            if ( amounts != null )
            {
                if ( amounts.Length != regs.Length )
                    Console.WriteLine( "{0} SpellInfo error: Incorrect reagents amounts" );
                else
                    m_Amounts = amounts;
            }
            else
