...
namespace Server.Spells
{
	public class SpellRegistry
	{
        ...
        // zombie - 18-06-2012 - niektore spelle dzialaja jak special movesy np. DeathStrike z Ninjitsu
        // dodanie aktywacji specjala po 2x clicku na scroll
        public static Spell NewSpell(int spellID, Mobile caster, Item scroll)
        {
            ...
            if (t != null)
            {
                if (t.IsSubclassOf(typeof(SpecialMove)))
                {
                    SpecialMove move = GetSpecialMove(spellID);

                    if ( SpecialMove.SetCurrentMove( caster, move ) )
                        scroll.Consume();
