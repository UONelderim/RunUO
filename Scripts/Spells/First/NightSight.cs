			...
			protected override void OnTarget( Mobile from, object targeted )
			{
                if (targeted is Mobile && m_Spell.CheckBSequence((Mobile)targeted))
				{
                    ...
                    if (targ.BeginAction(typeof(LightCycle)))
					{
                        ...
                        int bonus = (int)( Math.Min(1.0, from.Skills[SkillName.Magery].Value / 100) * NightSightPotion.NightSightBonus);

                        targ.LightLevel = bonus;
					...
					{
						from.SendMessage( "{0} postac ma juz wzmocniony wzrok.", from == targ ? "Twoja" : "Ta" );
					}
				}
