...
namespace Server.Spells.Bushido
{
	public abstract class SamuraiSpell : Spell
	{
		...
		public override bool CheckCast()
		{
			...
			if ( Caster.Skills[CastSkill].Value < RequiredSkill && !(this is Confidence) ) // Pozwol rzucac Confidence ponizej wymaganego progu
			{
		...
		public override bool CheckFizzle()
		{
			...
			// Sztuczny przyrost na Confidence ponizej 25 skilla:
			if (this is Confidence && Caster.Skills[CastSkill].Value < RequiredSkill )
			{
				if ( Caster.Mana < mana )
				{
					Caster.SendLocalizedMessage( 1060174, mana.ToString() ); // You must have at least ~1_MANA_REQUIREMENT~ Mana to use this ability.
				...
				if ( Caster.CheckSkill( CastSkill, 0, 30 ) )
				{
					Caster.Mana -= mana;
				}
				return false;
