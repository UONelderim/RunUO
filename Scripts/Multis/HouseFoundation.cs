...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
		...
		private static void TraceValidity( NetState state, int itemID )
		{
			try
			{
				using ( StreamWriter op = new StreamWriter( "Logi/comp_val.log", true ) )
		...
		public static void QueryDesignDetails( NetState state, PacketReader pvSrc )
		{
			...
			HouseFoundation foundation = World.FindItem( (Serial)pvSrc.ReadInt32() ) as HouseFoundation;
			...
			while( !Core.Closing )
			{
				...
				while( count > 0 )
				{
					...
					catch( Exception e )
					{
						...
						try
						{
							using( StreamWriter op = new StreamWriter( "Logi/dsd_exceptions.txt", true ) )
								op.WriteLine( e );
						}
						catch
						{
