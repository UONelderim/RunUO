...
namespace Server.Misc
{
	...
	public class WeightOverloading
	{
		...
		public static void FatigueOnDamage( Mobile m, int damage )
		{
			...
			double fatigueReduction = GetFatigueReduction(m);
		...
		public static double GetFatigueReduction(Mobile from) {
			double rating = 0.0;

			rating += GetArmorFatigueReductionValue(from.NeckArmor as BaseArmor);
			rating += GetArmorFatigueReductionValue(from.HandArmor as BaseArmor);
			rating += GetArmorFatigueReductionValue(from.HeadArmor as BaseArmor);
			rating += GetArmorFatigueReductionValue(from.ArmsArmor as BaseArmor);
			rating += GetArmorFatigueReductionValue(from.LegsArmor as BaseArmor);
			rating += GetArmorFatigueReductionValue(from.ChestArmor as BaseArmor);

			return 1 - (rating / 100);
		}

		private static double GetArmorFatigueReductionValue(BaseArmor ar) {
			if (ar == null)
				return 0.0;

			switch (ar.MaterialType) {
				default:
				case AMT.Cloth: return 2.0;
				case AMT.Leather:
				case AMT.Spined:
				case AMT.Horned:
				case AMT.Barbed:return 4.0;
				case AMT.Studded: return 5.0;
				case AMT.Bone: return 6.0;
				case AMT.Ringmail:return 7.0;
				case AMT.Chainmail:return 8.0;
				case AMT.Plate:return 10.0;
				case AMT.Dragon:return 12.0;
			}
		}
		...
		{
			//return ((( Core.ML && m.Race == Race.None) ? 100 : 40 ) + (int)(3.5 * m.Str));
		...
		public static void EventSink_Movement( MovementEventArgs e )
		{
			...
			if ( overWeight > 0 )
			{
				...
				//from.SendMessage(0x20, "_____TEST nadwaga: {0}", overWeight );
