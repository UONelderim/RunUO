	...
	public class InhumanSpeech
	{
			...
			get
			{
				if ( m_RatmanSpeech == null )
				{
					...
					m_RatmanSpeech.Responses = new string[]
						{
							"mieso", "zabic", "zbic", "zmiazdzyc", "mniam mniam",
							"chrupac", "zniszczyc", "zamordowac", "zjesc", "polknac",
							"masakra", "jedzenie", "potwor", "zly", "biec",
							"umrzec", "stracic", "glupi", "idiota", "glupiec", "szalony",
							"obiad", "jedzonko", "sniadanie", "walka", "bitwa",
							"pograzony", "rozerwac", "rozszarpac", "rozbic",
							"jadalny?", "strzep", "wypatroszyc", "brzydki", "smierdzacy",
							"durny", "paskudny", "cuchnacy", "smaczny", "najezdca",
							"attakowac", "najezdzac", "pladrowac", "grabic", "skarb",
							"przegrany", "przegrac", "szumowina"
						};
		...
		public bool OnSpeech( Mobile mob, Mobile speaker, string text )
		{
			...
			if ( keywordsFound.Count > 0 )
			{
				...
				switch ( Utility.Random( 6 ) )
				{
					...
					case 0:
					{
						response.Append( "Moja " ).Append( responseWord ).Append( '?' );
						break;
					}
					case 1:
					{
						response.Append( responseWord ).Append( " ty!" );
		...
		public void OnDamage( Mobile mob, int amount )
		{
			...
			if ( amount < 5 )
			{
				SayRandomTranslate( mob,
					"Bol!",
					"Moja nie zraniona za bardzo!",
					"Walczysz zle.",
					"Nic mnie nie robia twoje uderzenia!",
					"Ty nic nie wiesz o broni!" );
			}
			else
			{
				SayRandomTranslate( mob,
