...
namespace Server
{
	public class NameList
	{
		...
		public static string RandomName( string type )
		{
         ...
         // 30.06.2012 :: zombie
        public static string RandomName( Race race, bool female )
        {
            try
            {
                string type = String.Format( "{0}_{1}", race.Name.ToLower(), female ? "female" : "male" );

                return RandomName( type );
            }
            catch ( Exception e )
            {
                Console.WriteLine( "RandomName: " + e.ToString() );   
            }

            return RandomName( female ? "female" : "male" );
        }
        // zombie
