	...
	public class DataPath
	{
		...
		public static void Configure()
		{
			...
			if ( Config.CustomPath != null ) 
				Core.DataDirectories.Add( Config.CustomPath ); 
		...
		private static string GetExePath( string subName )
		{
			try
			{
				String keyString;
                if (Core.Is64Bit)
                    keyString = @"SOFTWARE\Wow6432Node\Origin Worlds Online\Ultima Online\KR Legacy Beta";
                else
                    keyString = @"SOFTWARE\Origin Worlds Online\Ultima Online\KR Legacy Beta";

                using (RegistryKey key = Registry.LocalMachine.OpenSubKey(String.Format(keyString)))
                {
                    if (key == null)
                        return null;
