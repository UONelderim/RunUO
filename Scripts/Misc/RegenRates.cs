...
namespace Server.Misc
{
	public class RegenRates
	{
		...
		private static TimeSpan Mobile_StamRegenRate( Mobile from )
		{
            ...
            {
                // cappedPoints = Math.Max(cappedPoints, ((PlayerMobile)from).staminaMeleeSkillRegenBonus());
		...
		private static TimeSpan Mobile_ManaRegenRate( Mobile from )
		{
			...
			if ( Core.AOS )
			{
				...
				//+2 mr
				double focusPoints = (int)(40 * 0.05);
                ...
                if (Core.ML && from is PlayerMobile)
                {
                    // cappedPoints = Math.Max(cappedPoints, ((PlayerMobile)from).manaMeleeSkillRegenBonus());
