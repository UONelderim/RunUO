...
namespace Server.Mobiles
{
	...
	public class EvolutionDragon3 : BaseCreature
	{
		...
		[Constructable]
		public EvolutionDragon3() : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			...
			m_PetLoyaltyTimer = new PetLoyaltyTimer3( this, TimeSpan.FromSeconds( 5.0 ) );
			m_PetLoyaltyTimer.Start();
			m_EndPetLoyalty = DateTime.Now + TimeSpan.FromSeconds( 5.0 );
		}

		public EvolutionDragon3(Serial serial) : base(serial)
		{
		}
			...
			else if ( this.Stage == 2 )
			{
				...
				if ( this.KP >= 7500 )
				{
					if ( this.S2 == true )
					{
			            ...
			            this.SetResistance( ResistanceType.Fire, 60, 65  );
			...
			else if ( this.Stage == 5 )
			{
				...
				if ( this.KP >= 775000 )
				{
					if ( this.S5 == true )
					{
			            ...
			            this.SetResistance( ResistanceType.Fire, 75, 80  );
			            this.SetResistance( ResistanceType.Poison, 20, 22 );
			            this.SetResistance( ResistanceType.Energy, 45, 46 );
				...
				if ( this.KP >= 1500000 )
				{
					if ( this.S6 == true )
					{
						...
						SetResistance( ResistanceType.Poison, 22, 25 );
			      		SetResistance( ResistanceType.Energy, 46, 50 );
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			return base.OnDragDrop( from, dropped );
                ...
                { 
                           	if ( obj is EvolutionDragon3 && obj is BaseCreature ) 
                           	{ 
					...
					if ( ed.Controlled == true && ed.ControlMaster == from )
					{
						if ( ed.Female == false )
						{
							if ( ed.AllowMating == true )
							{
								...
								m_MatingTimer = new MatingTimer3( this, TimeSpan.FromDays( 3.0 ) );
								m_MatingTimer.Start();
					...
					else if ( ed.Controlled == true )
					{
						if ( ed.Female == false )
						{
							if ( ed.AllowMating == true )
							{
								...
								{
									ed.ControlMaster.SendGump( new MatingGump3( from, ed.ControlMaster, this, ed ) );
					...
					if ( this.AllowMating == true )
					{
						if ( this.Pregnant == true )
						{
							if ( this.HasEgg == true )
							{
								...
								this.Pregnant = false;
								this.Blessed = false;
								from.AddToBackpack( new DragonEgg3() );
		...
		{
			private EvolutionDragon3 ed;
			private Mobile m_Mobile, m_From;

			public BreatheTimer( Mobile m, EvolutionDragon3 owner, Mobile from, TimeSpan duration ) : base( duration ) 
			{
				ed = owner;
				m_Mobile = m;
				m_From = from;
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch ( version )
			{
				case 1:
				{
					...
					m_MatingTimer = new MatingTimer3( this, m_EndMating - DateTime.Now );
					m_MatingTimer.Start();
					...
					m_PetLoyaltyTimer = new PetLoyaltyTimer3( this, m_EndPetLoyalty - DateTime.Now );
					m_PetLoyaltyTimer.Start();
				...
				case 0:
				{
					...
					m_MatingTimer = new MatingTimer3( this, durationmating );
					m_MatingTimer.Start();
					m_EndMating = DateTime.Now + durationmating;

					m_PetLoyaltyTimer = new PetLoyaltyTimer3( this, durationloyalty );
					m_PetLoyaltyTimer.Start();
	...
	public class MatingTimer3 : Timer
	{ 
		private EvolutionDragon3 ed;

		public MatingTimer3( EvolutionDragon3 owner, TimeSpan duration ) : base( duration ) 
		{ 
	...
	public class PetLoyaltyTimer3 : Timer
	{ 
		private EvolutionDragon3 ed;

		public PetLoyaltyTimer3( EvolutionDragon3 owner, TimeSpan duration ) : base( duration ) 
		{ 
			Priority = TimerPriority.OneSecond;
			ed = owner;
		}
		...
		protected override void OnTick() 
		{
			...
			PetLoyaltyTimer3 lt = new PetLoyaltyTimer3( ed, TimeSpan.FromSeconds( 5.0 ) );
			lt.Start();
	...
	public class MatingGump3 : Gump
	{
		private Mobile m_From;
		private Mobile m_Mobile;
		private EvolutionDragon3 m_ED1;
		private EvolutionDragon3 m_ED2;

		public MatingGump3( Mobile from, Mobile mobile, EvolutionDragon3 ed1, EvolutionDragon3 ed2 ) : base( 25, 50 )
		{
			Closable = false; 
			Dragable = false; 
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				MatingTimer3 mt = new MatingTimer3( m_ED1, TimeSpan.FromDays( 3.0 ) );
				mt.Start();
