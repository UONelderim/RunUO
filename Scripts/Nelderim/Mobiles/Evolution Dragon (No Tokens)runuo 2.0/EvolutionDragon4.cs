...
namespace Server.Mobiles
{
	...
	public class EvolutionDragon4 : BaseCreature
	{
		...
		[Constructable]
		public EvolutionDragon4() : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			...
			m_PetLoyaltyTimer = new PetLoyaltyTimer4( this, TimeSpan.FromSeconds( 5.0 ) );
			m_PetLoyaltyTimer.Start();
			m_EndPetLoyalty = DateTime.Now + TimeSpan.FromSeconds( 5.0 );
		}

		public EvolutionDragon4(Serial serial) : base(serial)
		{
		}
			...
			if ( this.Stage == 1 )
			{
				...
				if ( this.KP >= 25000 )
				{
					if ( this.S1 == true )
					{
			            ...
			            this.SetResistance( ResistanceType.Energy, 0, 5 );
			...
			else if ( this.Stage == 5 )
			{
				...
				if ( this.KP >= 7750000 )
				{
					if ( this.S5 == true )
					{
			            ...
			            this.SetResistance( ResistanceType.Cold, 45, 46 );
			            this.SetResistance( ResistanceType.Fire, 30, 32  );
			            this.SetResistance( ResistanceType.Poison, 75, 80 );
			            this.SetResistance( ResistanceType.Energy, 20, 22 );
			...
			else if ( this.Stage == 6 )
			{
				...
				if ( this.KP >= 15000000 )
				{
					if ( this.S6 == true )
					{
						...
						SetResistance( ResistanceType.Cold, 46, 50 );
						SetResistance( ResistanceType.Poison, 80, 85 );
			      		SetResistance( ResistanceType.Energy, 22, 25 );
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			return base.OnDragDrop( from, dropped );
                ...
                { 
                           	if ( obj is EvolutionDragon4 && obj is BaseCreature ) 
                           	{ 
					...
					if ( ed.Controlled == true && ed.ControlMaster == from )
					{
						if ( ed.Female == false )
						{
							if ( ed.AllowMating == true )
							{
								...
								m_MatingTimer = new MatingTimer4( this, TimeSpan.FromDays( 3.0 ) );
								m_MatingTimer.Start();
					...
					else if ( ed.Controlled == true )
					{
						if ( ed.Female == false )
						{
							if ( ed.AllowMating == true )
							{
								...
								{
									ed.ControlMaster.SendGump( new MatingGump4( from, ed.ControlMaster, this, ed ) );
					...
					if ( this.AllowMating == true )
					{
						if ( this.Pregnant == true )
						{
							if ( this.HasEgg == true )
							{
								...
								this.Pregnant = false;
								this.Blessed = false;
								from.AddToBackpack( new DragonEgg4() );
		...
		{
			private EvolutionDragon4 ed;
			private Mobile m_Mobile, m_From;

			public BreatheTimer( Mobile m, EvolutionDragon4 owner, Mobile from, TimeSpan duration ) : base( duration ) 
			{
				ed = owner;
				m_Mobile = m;
				m_From = from;
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch ( version )
			{
				case 1:
				{
					...
					m_MatingTimer = new MatingTimer4( this, m_EndMating - DateTime.Now );
					m_MatingTimer.Start();
					...
					m_PetLoyaltyTimer = new PetLoyaltyTimer4( this, m_EndPetLoyalty - DateTime.Now );
					m_PetLoyaltyTimer.Start();
				...
				case 0:
				{
					...
					m_MatingTimer = new MatingTimer4( this, durationmating );
					m_MatingTimer.Start();
					m_EndMating = DateTime.Now + durationmating;

					m_PetLoyaltyTimer = new PetLoyaltyTimer4( this, durationloyalty );
					m_PetLoyaltyTimer.Start();
	...
	public class MatingTimer4 : Timer
	{ 
		private EvolutionDragon4 ed;

		public MatingTimer4( EvolutionDragon4 owner, TimeSpan duration ) : base( duration ) 
		{ 
	...
	public class PetLoyaltyTimer4 : Timer
	{ 
		private EvolutionDragon4 ed;

		public PetLoyaltyTimer4( EvolutionDragon4 owner, TimeSpan duration ) : base( duration ) 
		{ 
			Priority = TimerPriority.OneSecond;
			ed = owner;
		}
		...
		protected override void OnTick() 
		{
			...
			PetLoyaltyTimer4 lt = new PetLoyaltyTimer4( ed, TimeSpan.FromSeconds( 5.0 ) );
			lt.Start();
	...
	public class MatingGump4 : Gump
	{
		private Mobile m_From;
		private Mobile m_Mobile;
		private EvolutionDragon4 m_ED1;
		private EvolutionDragon4 m_ED2;

		public MatingGump4( Mobile from, Mobile mobile, EvolutionDragon4 ed1, EvolutionDragon4 ed2 ) : base( 25, 50 )
		{
			Closable = false; 
			Dragable = false; 
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				MatingTimer4 mt = new MatingTimer4( m_ED1, TimeSpan.FromDays( 3.0 ) );
				mt.Start();
