...
namespace Nelderim.Engines.ChaosChest
{
    public class ChaosChest : LockableContainer
    {
        ...
        private static Item Kolczyki(int type)
        {
            ...
            switch (Utility.Random(2))
            {
                case 1: earrings = new SilverEarrings();
                    break;
                default: earrings = new GoldEarrings();
                    break;
            }
            // Ustawianie resista
            Array resistanceTypes = Enum.GetValues(typeof(AosElementAttribute));
            AosElementAttribute res = (AosElementAttribute) resistanceTypes.GetValue(Utility.Random(resistanceTypes.Length));
            earrings.Resistances[res] = 5;
            
            //Ustawianie mr/hr/sr
            switch (Utility.Random(3))
            {
                case 2: earrings.Attributes[AosAttribute.RegenMana] = 2;
                    break;
                case 1: earrings.Attributes[AosAttribute.RegenHits] = 2;
                    break;
                default: earrings.Attributes[AosAttribute.RegenStam] = 2;
                    break;
            }

            TemporaryQuestObject tqo = new TemporaryQuestObject("Bardzo chaotyczne kolczyki", (double) 10 * 24 * 60 /*10dni*/ );
