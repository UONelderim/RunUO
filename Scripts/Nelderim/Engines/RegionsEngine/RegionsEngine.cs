// 08.07.2012 :: zombie :: przebudowa ArkoSystemu
     ...
     //                    }
					...
					if ( reg.RegionPopulation != null )
					{
						...
						for (int i = 0; i < Race.AllRaces.Count; i++) {
							xml.WriteAttributeString(Race.AllRaces[i].Name, XmlConvert.ToString(reg.RegionPopulation.Proportions[i]));
						}
						...
						for ( int i = 0; i < reg.Guards.Length; i++ )
						{
							...
							if ( g != null )
							{
								...
								for (int j = 0; j < Race.AllRaces.Count; j++) {
									xml.WriteAttributeString(Race.AllRaces[j].Name, XmlConvert.ToString(g.Races[j]));
								}
		...
		public static bool ActIntolerativeHarmful( Mobile source, Mobile target, bool msg )
		{
			try
			{
				if( source != null && target != null && source.InLOS(target))
				{
                    ...
                    double intolerance = (double)region.Intolerance[ Race.AllRaces.IndexOf(target.Race) ];
