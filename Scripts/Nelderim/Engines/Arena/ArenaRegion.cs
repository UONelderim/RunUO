 ...
 *
 ***************************************************************************/
// 06.01.22 :: troyan :: naprawa bledu z wyrzucaniem z gry przy treningowym wskrzeszaniu
// 06.02.02 :: troyan :: modyfikacje funkcji bankowych
// 06.02.25 :: troyan :: dodatkowa protekcja
// 06.02.25 :: troyan :: likwidacja bledu z wielokrotnym treningiem blokujacym arene
// 07.02.28 :: emfor :: uwzglednienie wystepowania bushido i ninjitsu
// 07.05.12 :: emfor :: Kiedy nie ma zakazu wprowadzania mountow, po zrzuceniu z konia, zwierze jest usuwane poza arene
	...
	public class ArenaRegion : Region
	{
		...
		{		
			...
			protected override void OnTick()
			{
				...
				if ( m_Target.Region is ArenaRegion && ( m_Target.Region as ArenaRegion ).IsFighter( m_Target ) )
				{
					m_Target.SendLocalizedMessage( 505256 ); // Uplynal wykupiony czas na arenie.
					( m_Target.Region as ArenaRegion ).EndFight( m_Target, EOFReason.EndOfTime );
				}
			}
		}
		...
		{		
			...
			{
				ArenaRegion.Protection.Add( target );
				m_Target = target;
			...
			protected override void OnTick()
			{
				...
				ArenaRegion.Protection.Remove( m_Target );
			}
		}
		...
		public class Fighter
		{
			...
			public ArenaRegion Arena { get { return m_Owner; } }
			public FightTimer Timer { get { return m_Timer; } }
			
			public Fighter( ArenaRegion a, Mobile m, FightType ft )
			{
				m_Owner = a;
				m_Mobile = m;
				m_Fight = ft;
		...
		public ArenaRegion( string name, Map map, Rectangle3D[] area, ArenaTrainer owner ) : base( owner.ArenaName, owner.Map, 100, area )
		{
			m_Owner = owner;
				...
				if ( IsFighter( m ) )
				{
					...
					if ( m is BaseCreature && !( m is BaseMount ) )
					{
						...
						if ( m_Owner.NoFamiliars && m is BaseFamiliar )
						{
							...
							ArenaRegion.Unsummon( m );
						}
						else if ( m_Owner.NoSummons && ArenaRegion.IsSummon( m ) )
						{
							cm = bc.SummonMaster;
						...
						else if ( m_Owner.NoControlledSummons && ArenaRegion.IsControlledSummon( m ) )
						{
							if ( cm != null )
								cm.SendLocalizedMessage( 505268 );
						...
						else if ( m_Owner.NoControls && !( ArenaRegion.IsControlledSummon( m ) || ArenaRegion.IsSummon( m )
						                                 || m is BaseFamiliar ) )
						{
							if ( cm != null )
								cm.SendLocalizedMessage( 505268 );
    ...
    */
		...
		public override void OnDeath( Mobile m )
		{
			...
			try
			{
				...
				{
					if ( m is BaseCreature && !( m is BaseFamiliar || ArenaRegion.IsSummon( m ) || ArenaRegion.IsControlledSummon( m ) ) )
					{
						ArenaRegion.Rejuvenate( m, false );
						ArenaRegion.Teleport( m, m_Owner.EndOfFightPoint );
						ClearAgressors( m );
					...
					else
					{
						switch ( ft )
						{
							...
							case FightType.LongTraining:
								{
									ArenaRegion.Rejuvenate( m, false );
									ArenaRegion.RejuvenateEffect( m );
		...
		public bool AddFighter( Mobile blue, Mobile red, FightType ft )
		{
			try
			{
				...
				ArenaRegion.Teleport( blue, m_Owner.CornerBlue );
				ArenaRegion.Teleport( red, m_Owner.CornerRed );
				...
				{
					if ( mob is BaseFamiliar || ArenaRegion.IsSummon( mob ) || ArenaRegion.IsControlledSummon( mob ) )
						toUnsummon.Add( mob );
					else if ( ArenaRegion.IsControlled( mob, blue ) && !Owner.NoControls )
						toMove.Add( mob );
				}
				...
				foreach ( Mobile mob in eable )
				{
					if ( mob is BaseFamiliar || ArenaRegion.IsSummon( mob ) || ArenaRegion.IsControlledSummon( mob ) )
						toUnsummon.Add( mob );
					else if ( ArenaRegion.IsControlled( mob, red ) && !Owner.NoControls )
						toMove.Add( mob );
				}
				...
				for ( int i = toUnsummon.Count - 1; i >= 0; i-- )
					ArenaRegion.Unsummon( toUnsummon[ i ] as Mobile );
		...
		public bool AddFighter( TournamentFight tf )
		{
			try
			{
				...
				foreach ( Mobile mob in eable )
				{
					if ( mob is BaseFamiliar || ArenaRegion.IsSummon( mob ) || ArenaRegion.IsControlledSummon( mob ) )
						toUnsummon.Add( mob );
					else if ( ArenaRegion.IsControlled( mob, blue ) )
						toMove.Add( mob );
				}
			...
			try
			{
				if ( m != null && IsFighter( m, out ft ) )
				{
					...
					if ( m.Followers > 0 )
					{
						...
						{
							if ( ArenaRegion.IsControlled( mob, m ) || ArenaRegion.IsSummoned( mob, m ) )
							{
								#region Dispell summonow
								if ( ArenaRegion.IsSummon( mob ) ||ArenaRegion.IsControlledSummon( mob ) )
									toDelete.Add( mob );
								else
								#endregion
								{
						...
						{
							ArenaRegion.Unsummon( toDelete[i] as Mobile );
							toDelete.RemoveAt( i );
						}
						...
						if ( m.Followers > 0 )
						{
							...
							foreach ( Mobile mob in eable )
							{
								if ( ArenaRegion.IsControlled( mob, m ) && mob.Frozen )
									mob.Frozen = false;
							}
						}
					}
					...
					if ( ( int ) reason > ( int ) EOFReason.Exit )
					{
						...
						if ( reason != EOFReason.Victory )
						{
							...
							foreach ( Mobile mob in GetMobiles() )
							{
								if ( ArenaRegion.IsControlled( mob, m ) )
									move.Add( mob );
							}
		...
		private void ClearAgressors( Mobile m )
		{
			...
			try
			{
				...
				foreach ( Mobile mobile in GetMobiles() )
				{
					if ( mobile != m && IsFighter( mobile ) && mobile is Revenant && ( mobile as Revenant ).RevenantTarget == m )
						rev.Add( mobile );
				}
						
				foreach ( Mobile mobile in rev )
					ArenaRegion.Unsummon( mobile );
			}
			catch ( Exception exc )
			{
				Console.WriteLine( exc.ToString() );
			...
			try
			{
				if ( m != null && m.AccessLevel == AccessLevel.Player )
				{
					...
					if ( this.Owner.NoEnter )
						ArenaRegion.Teleport( m, this.Owner.ExtortPoint );
				}
			}
			catch ( Exception exc )
			{
		...
		public void CleanArena( string message )
		{
			try
			{
				...
				for ( int i = 0; i < toExtort.Count; i++ )
					ArenaRegion.Teleport( toExtort[ i ] as Mobile, m_Owner.ExtortPoint );
			}
			catch ( Exception exc )
			{
				Console.WriteLine( exc.ToString() );
		...
		public static bool CanSee( Mobile from, Mobile target )
		{
			...
			ArenaRegion fr = from.Region as ArenaRegion;
			bool fif = ( fr != null && fr.IsFighter( from ) );
						  
			if ( fif )
			{
				// Console.WriteLine( "{0} is fighter", from.Name );
				ArenaRegion tr = target.Region as ArenaRegion;
