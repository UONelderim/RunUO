...
{ 
	...
	class WeedHelper
	{
        ...
        private static bool ValidateGardenPlot(Map map, Point3D p)
        {
            ...
            if (!ground)
            {
                Tile[] tiles = map.Tiles.GetStaticTiles(p.X, p.Y);
		...
		{
			List<Tile> zTiles = new List<Tile>();
			Tile[] allTiles = map.Tiles.GetStaticTiles(p.X, p.Y, true);
		...
		private static bool IsTileFloor(Tile tile)
		{
			int tileID = tile.ID & 0x3FFF;
