				...
				if ( packs != null )
				{
					for ( int i = 0; i < packs.Count; ++i )
					{
						...
						if ( built is Item ) 
						{
							...
							if ( Config.ValidateLabeling( built ) ) 
							{
								((Item)built).LabelOfCreator = CommandLogging.Format( from ) as string;
								((Item)built).ModifiedBy = from.Account.Username;
								((Item)built).ModifiedDate = DateTime.Now;
							}
						...
						else if ( built is Mobile ) 
						{
							...
							if ( Config.ValidateLabeling( built ) ) 
							{
								m.LabelOfCreator = CommandLogging.Format( from ) as string;
								m.ModifiedBy = from.Account.Username;
					...
					for ( int x = start.X; x <= end.X; ++x )
					{
						for ( int y = start.Y; y <= end.Y; ++y )
						{
							...
							if ( built is Item )
							{
								...
								if ( Config.ValidateLabeling( item ) ) 
								{
