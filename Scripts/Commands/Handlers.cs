...
namespace Server.Commands
{
	public class CommandHandlers
	{
		...
		public static void WriteProfiles_OnCommand( CommandEventArgs e )
		{
			try
			{
				using ( StreamWriter sw = new StreamWriter( "Logi/profiles.log", true ) )
				{
		...
		public static void DumpTimers_OnCommand( CommandEventArgs e )
		{
			try
			{
				using ( StreamWriter sw = new StreamWriter( "Logi/timerdump.log", true ) )
					Timer.DumpInfo( sw );
			}
			catch
			{
		...
		{
			using ( StreamWriter op = new StreamWriter( "Logi/objects.log" ) )
			{
		...
		public static void TraceExpanded_OnCommand( CommandEventArgs e )
		{
			...
			try
			{
				using ( StreamWriter op = new StreamWriter( "Logi/expandedItems.log", true ) )
				{
		...
		public static void TraceInternal_OnCommand( CommandEventArgs e )
		{
			...
			using ( StreamWriter op = new StreamWriter( "Logi/internal.log" ) )
			{
		...
		{
			ProfileWorld( "items", "Logi/worldprofile_items.log" );
			ProfileWorld( "mobiles", "Logi/worldprofile_mobiles.log" );
		}
		...
		private static void Go_OnCommand( CommandEventArgs e )
		{
			...
			if ( e.Length == 1 )
			{
				...
				{
					//[go serial
					Serial ser = (Serial)e.GetInt32( 0 );
					...
					else
					{
						...
						foreach (NetState ns in NetState.Instances) {
							...
							int index = m.Name.IndexOf(' ');

							String playerName = (index != -1) ? m.Name.Substring(0, index) : m.Name;

							if (Insensitive.Equals(playerName, name)) {
						...
						if ( ser != 0 )
							from.SendLocalizedMessage(505705); // Nie znaleziono obiektu z podanym numerem seryjnym (Serial).
						else
							from.SendLocalizedMessage(505706); // Nie znaleziono gracza, ani regionu o podanej nazwie.
