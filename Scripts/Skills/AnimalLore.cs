...
namespace Server.SkillHandlers
{
	public class AnimalLore
	{
        ...
        private static void Lore(Mobile from, BaseCreature c, bool shrunk)
        {
            if (!c.IsDeadPet)
            {
                if (c.Body.IsAnimal || c.Body.IsMonster || c.Body.IsSea)
                {
                    if ((!c.Controlled || !c.Tamable) && !shrunk && from.Skills[SkillName.AnimalLore].Base < 100.0)
                    {
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is BaseCreature )
				{
                    ...
                    AnimalLore.Lore(from, c, false);
				}
                else if (targeted is ShrunkPet)
                {
                    ShrunkPet sp = (ShrunkPet)targeted;
                    if (sp.Pet != null)
                        AnimalLore.Lore(from, sp.Pet, true);
                }
				else
				{
					from.SendLocalizedMessage( 500329 ); // That's not an animal!
	...
	public class AnimalLoreGump : Gump
	{
		...
		public AnimalLoreGump( BaseCreature c ) : base( 250, 50 )
		{
            ...
            #region klejnoty
                ...
                foodPrefS = "szafir i gwiezdny szafir";
            ...
            if( foodPrefS == null )
			    ...
			    AddHtml( 153, 168, 160, 18, foodPrefS, false, false );
