...
namespace Server.SkillHandlers
{
	public class Snooping
	{
        ...
        {
            CommandSystem.Register("snoop", AccessLevel.Player, new CommandEventHandler(Snoop_Command));
        }

        [Usage("snoop")]
        [Description("Uzywa umiejetnosci Zagladanie na ekwipunku wskazanego stworzenia")]
        private static void Snoop_Command(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(8, false, TargetFlags.None, new TargetCallback(Snoop_Command_OnTarget));
        }

        private static void Snoop_Command_OnTarget(Mobile from, object obj)
        {
            Mobile m = obj as Mobile;
            if (m != null && m != from )
            {
                if( m.Backpack != null )
                    Container_Snoop(m.Backpack, from);
            }
        }
