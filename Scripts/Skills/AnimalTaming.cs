			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is Mobile )
				{
					if ( targeted is BaseCreature )
					{
						...
						else if ( CheckMastery( from, creature ) || from.Skills[SkillName.AnimalTaming].Value >= (creature.MinTameSkill + (creature.Owners.Count * 6.0)))
						{
						...
						else
						{
							...
							{
								creature.PrivateOverheadMessage(MessageType.Regular, 0x3B2, 3010198, from.NetState); // You have no chance of taming this creature - tamed by someone else
							}
                            else
                            {
