		...
		private class SpiritSpeakSpell : Spell
		{
			...
			public override void OnCast()
			{
				...
				if ( toChannel != null )
				{
					...
					max = min + 4;
					mana = 0;
					number = 1061287; // Laczysz sie z energia duchowa pobliskich zwlok leczac swoje rany.
				}
				else
				{
					min = 1 + (int)(Caster.Skills[SkillName.SpiritSpeak].Value * 0.25);
					max = min + 4;
					mana = 10;
					number = 1061286; // Laczysz sie z wlasna energia duchowa leczac swoje rany.
				}

				if ( Caster.Mana < mana )
				{
					Caster.SendLocalizedMessage( 1061285 ); // Brakuje ci many na uzycie tej umiejetnosci.
				}
				else
				{
					...
					{
						Caster.SendLocalizedMessage( 502443 ); // Proba kontaktu ze swiatem umarlych nie powiodla sie.
					}
					else
					{
						if ( toChannel != null )
