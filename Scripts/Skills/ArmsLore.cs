	...
	public class ArmsLore
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				// Tinker Traps (Start).
                // =====================
                else if ( targeted is BaseTinkerTrap )
                {
                    BaseTinkerTrap tt = (BaseTinkerTrap)targeted;

                    if ((from.CheckTargetSkill(SkillName.ArmsLore, targeted, 0.0, 100.0)) || (tt.Owner == from))
                    {
                        StringBuilder sb = new StringBuilder();
                        if (tt.Owner == null)
                            sb.Append("Ta pułapka jest nienaruszona.");

                        else if (tt.Owner == from)
                        {
                            if (tt.TrapArmed)
                                sb.Append("Ustawiasz pułapkę.");
                            else
                                sb.Append("To jest Twoja pułapka.");
                        }
                        else
                        {
                            if (tt.TrapArmed)
                                sb.Append("Ta pułapka została ustawiona przez ");
                            else
                                sb.Append("Ta pułapka należy do ");
