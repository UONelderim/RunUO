...
namespace Server.Network
{
	...
	public class PacketHandlers
	{
		...
		{
			IEntity e = World.FindEntity( (Serial)pvSrc.ReadInt32() );
		...
		public static void RenameRequest( NetState state, PacketReader pvSrc )
		{
			...
			Mobile targ = World.FindMobile( (Serial)pvSrc.ReadInt32() );
		...
		public static void VendorBuyReply( NetState state, PacketReader pvSrc )
		{
			...
			Mobile vendor = World.FindMobile( (Serial)pvSrc.ReadInt32() );
		...
		public static void VendorSellReply( NetState state, PacketReader pvSrc )
		{
			...
			if ( count < 100 && pvSrc.Size == (1+2+4+2+(count*6)) )
			{
				...
				{
					Item item = World.FindItem( (Serial)pvSrc.ReadInt32() );
					int Amount = pvSrc.ReadInt16();
			...
			switch ( type )
			{
				...
				case 0x27: // Cast spell from book
				{
					...
					if ( split.Length > 0 )
					{
						...
						EventSink.InvokeCastSpellRequest( new CastSpellRequestEventArgs( m, spellID, World.FindItem( (Serial)serial ) ) );
					}
		...
		public static void EquipReq( NetState state, PacketReader pvSrc )
		{
			...
			Mobile to = World.FindMobile( (Serial)pvSrc.ReadInt32() );
		...
		public static void DropReq( NetState state, PacketReader pvSrc )
		{
			...
			int y = pvSrc.ReadInt16();
			int z = pvSrc.ReadSByte();
			Serial dest = (Serial)pvSrc.ReadInt32();
		...
		public static void DisplayGumpResponse( NetState state, PacketReader pvSrc ) {
			...
			if ( typeID == 461 ) { // Virtue gump
				...
				if ( buttonID == 1 && switchCount > 0 ) {
					Mobile beheld = World.FindMobile( (Serial)pvSrc.ReadInt32() );

					if ( beheld != null ) {
						EventSink.InvokeVirtueGumpRequest( new VirtueGumpRequestEventArgs( state.Mobile, beheld ) );
					}
				} else {
					Mobile beheld = World.FindMobile( (Serial)serial );
		...
		public static void LookReq( NetState state, PacketReader pvSrc )
		{
			...
			Serial s = (Serial)pvSrc.ReadInt32();
		...
		public static void CastSpell( NetState state, PacketReader pvSrc )
		{
			...
			if ( pvSrc.ReadInt16() == 1 )
				spellbook = World.FindItem( (Serial)pvSrc.ReadInt32() );
		...
		{
			if ( PartyCommands.Handler != null )
				PartyCommands.Handler.OnRemove( state.Mobile, World.FindMobile( (Serial)pvSrc.ReadInt32() ) );
		}

		public static void PartyMessage_PrivateMessage( NetState state, PacketReader pvSrc )
		{
			if ( PartyCommands.Handler != null )
				PartyCommands.Handler.OnPrivateMessage( state.Mobile, World.FindMobile( (Serial)pvSrc.ReadInt32() ), pvSrc.ReadUnicodeStringSafe() );
		}
		...
		{
			if ( PartyCommands.Handler != null )
				PartyCommands.Handler.OnAccept( state.Mobile, World.FindMobile( (Serial)pvSrc.ReadInt32() ) );
		}

		public static void PartyMessage_Decline( NetState state, PacketReader pvSrc )
		{
			if ( PartyCommands.Handler != null )
				PartyCommands.Handler.OnDecline( state.Mobile, World.FindMobile( (Serial)pvSrc.ReadInt32() ) );
		}
		...
		public static void ContextMenuResponse( NetState state, PacketReader pvSrc )
		{
			...
			if ( from != null )
			{
				...
				{
					IEntity entity = World.FindEntity( (Serial)pvSrc.ReadInt32() );
		...
		{
			Mobile from = state.Mobile;
			IEntity target = World.FindEntity( (Serial)pvSrc.ReadInt32() );
