		...
		private void CheckListener()
		{
			for ( int j = 0; j < m_Listeners.Length; ++j )
			{
				...
				for ( int i = 0; i < accepted.Length; ++i )
				{
					...
					if ( ns.Running )
						Console.WriteLine( "Polaczenie od: {0}: Polaczony. [{1} Online]", ns, NetState.Instances.Count );
				}
			}
		}
			...
			lock ( buffer )
			{
				...
				if ( !ns.Seeded )
				{
					...
					else if ( buffer.Length >= 4 )
					{
						...
						if ( seed == 0 )
						{
							Console.WriteLine( "Polaczenie od: {0}: Wykryto niezgodnego clienta, rozlaczono.", ns );
							ns.Dispose();
							return false;
						}
				...
				while ( length > 0 && ns.Running )
				{
					...
					{
                        Console.WriteLine("Polaczenie od: {0}: Wykryto zakodowanego clienta, rozlaczono.", ns);
						ns.Dispose();
						break;
					}
