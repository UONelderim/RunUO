...
namespace Server.Network {
	...
	public class NetState {
		...
		public void WriteConsole( string text ) {
			Console.WriteLine( "Polaczenie od: {0}: {1}", this, text );
		}
		...
		public virtual void Send( Packet p ) {
			...
			if ( buffer != null ) {
				...
				try {
					...
					Console.WriteLine( "Polaczenie od: {0}: Za duzo danych, rozlaczono.", this );
					Dispose( false );
				}
				...
				using ( StreamWriter op = new StreamWriter( "Logi/null_send.log", true ) )
				{
		...
		public static void TraceException( Exception ex ) {
			try {
				using ( StreamWriter op = new StreamWriter( "Logi/network-errors.log", true ) ) {
		...
		public static void ProcessDisposedQueue() {
			...
			while ( breakout < 200 && m_Disposed.Count > 0 ) {
				...
				if ( a != null ) {
					ns.WriteConsole( "Rozlaczono: [{0} Online] [{1}]", m_Instances.Count, a );
				} else {
					ns.WriteConsole( "Rozlaczono: [{0} Online]", m_Instances.Count );
				}
			}
		}
