	...
	public static class World {
		...
		private sealed class GuildEntry : IEntityEntry {
			...
			public Serial Serial {
				get {
					return (Serial)(m_Guild == null ? 0 : m_Guild.Id);
				}
			}
		...
		public static void Load() {
			...
			Console.WriteLine( "Swiat: Wczytywanie..." );
			...
			Console.WriteLine( "Gotowe ({1} items, {2} mobiles) ({0:F2} seconds)", watch.Elapsed.TotalSeconds, m_Items.Count, m_Mobiles.Count );
		}
		...
		private static void AppendSafetyLog( string action, IEntity entity ) {
			...
			try {
				using ( StreamWriter op = new StreamWriter( "Logi/world-save-errors.log", true ) ) {
		...
		public static void Save( bool message, bool permitBackgroundWrite ) {
			...
			Console.WriteLine( "Zapis zakonczony, trwal {0:F2} sekundy.", watch.Elapsed.TotalSeconds );

			if ( message )
				Broadcast( 0x35, true, "Zapis swiata zostal zakonczony, trwal {0:F1} sekundy.", watch.Elapsed.TotalSeconds );
