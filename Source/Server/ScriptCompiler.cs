...
namespace Server
{
	public class ScriptCompiler
	{
		...
		public static bool CompileCSScripts( bool debug, bool cache, out Assembly assembly )
		{
			...
			{
				if( cache && File.Exists( "Output/Scripts.CS.hash" ) )
				{
					try
					{
						byte[] hashCode = GetHashCode( "Output/Scripts.CS.dll", files, debug );
						...
						using( FileStream fs = new FileStream( "Output/Scripts.CS.hash", FileMode.Open, FileAccess.Read, FileShare.Read ) )
						{
							using( BinaryReader bin = new BinaryReader( fs ) )
							{
								...
								if( bytes.Length == hashCode.Length )
								{
									...
									if( valid )
									{
										assembly = Assembly.LoadFrom( "Output/Scripts.CS.dll" );
...
// #else
				...
				if( cache && Path.GetFileName( path ) == "Scripts.CS.dll" )
				{
					try
					{
						...
						using( FileStream fs = new FileStream( "Output/Scripts.CS.hash", FileMode.Create, FileAccess.Write, FileShare.None ) )
						{
		...
		public static void Display( CompilerResults results )
		{
			if( results.Errors.Count > 0 )
			{
				...
				if( errors.Count > 0 )
					Console.WriteLine( "Bledy ({0} errors, {1} warnings)", errors.Count, warnings.Count );
				else
					Console.WriteLine( "Gotowe ({0} errors, {1} warnings)", errors.Count, warnings.Count );
		...
		public static void DeleteFiles( string mask )
		{
			try
			{
				string[] files = Directory.GetFiles( Path.Combine( Core.BaseDirectory, "Output" ), mask );
		...
		public static bool Compile( bool debug, bool cache )
		{
			...
			Console.WriteLine( "Gotowe ({0} items, {1} mobiles)", Core.ScriptItems, Core.ScriptMobiles );
